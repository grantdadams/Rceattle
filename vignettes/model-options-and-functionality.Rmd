...
| `3` | `"FishedNonEquilibrium"` | Fished non-equilibrium with init devs; F<sub>init</sub> estimated inside geometric series |
| `4` | `"FishedNonEquilibriumScaled"` | Fished non-equilibrium with R<sub>0</sub>-scaled F<sub>init</sub> after geometric series|
...
indexed over `N_sel_bins` and can be normalised by `Sel_norm_bin1` or between
...
If `Selectivity` is `AR1` or `Environmental`, `Time_varying_q` can be a integer or character of integers specifying the environmental series in `env_data` to use (e.g. `Time_varying_q = 1` or `Time_varying_q = "1,2,3"`).
