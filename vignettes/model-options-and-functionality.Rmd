...
Priors can be added to stock-functions via `build_srr()`. See `srr_est_mode` in `?build_srr` for options. 
Use `srr_indices` to pick columns in `env_data` (e.g. `srr_indices = c(2,3,4)`).
...
| `4` | Environmentally driven, sex-invariant. M1<sub>spp,yr</sub> = M1<sub>spp</sub> · exp(X · β<sub>spp</sub>); use `M1_indices` to pick columns in `env_data` |
