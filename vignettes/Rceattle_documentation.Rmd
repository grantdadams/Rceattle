...
Multiple age-based selectivity functions are supported by Rceattle. They are controlled by `Selectivity_index`, `Selectivity`, `N_sel_bins`, `Time_varying_sel`, `Sel_sd_prior`, and `Age_first_selected` in fleet_control sheet of an Rceattle data file. They are defined as follows:
...
Input:
`Selectivity = 2`, `N_sel_bins = 6`, `Time_varying_sel = 12.5`, `Sel_sd_prior = 200`, and `Age_first_selected = 1`
...
Input:
`Selectivity = 5`, `N_sel_bins = 6`, `Time_varying_sel = NA`,  `Sel_sd_prior = NA`, and `Age_first_selected = `$A_{min}$
