...
Once a model is fitted, Rceattle can project the stock forward under alternative fishing mortality and recruitment assumptions. This vignette covers:
...
`estimateMode = 2` runs the projection period only, holding all estimated parameters fixed, under a specified harvest control rule. `build_hcr(HCR = 2, Ftarget = ...)` sets a constant F for each species.

When several fisheries target the same species, the `Proj_F_proportion` column in `fleet_control` splits projected F across those fleets. The values must sum to 1 per species.
...
Projection recruitment deviations live in the parameter list, so you can overwrite them directly before projecting. `estimateMode = 3` evaluates the model without re-estimating anything, so only the updated deviations change the projection.
...
Under a DSEM the deviations are not independent, so they are not bootstrapped: the projection is drawn from the fitted GMRF conditional on everything the model already knows, including any environmental covariate supplied over the projection. That needs `build_DSEM(estimate_projection = TRUE)`, so the latent states span the projection; `sample_rec()` stops and names the setting if it is missing. Estimating the projection also overrides `build_srr(proj_mean_rec = )`, since recruitment projected at its historical mean would ignore the SEM's projection states.
...
Setting `random_rec = TRUE` treats log-recruitment deviations as random effects, marginalized via the Laplace approximation, rather than as penalized fixed effects. This carries recruitment process uncertainty through into the projections.
