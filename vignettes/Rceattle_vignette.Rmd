...
Currently `RCeattle` can take survey index, survey catch-at-age, fishery catch, fishery catch-at-age, bioenergetics, life history, and diet based inputs and estimates time series of biomass, fishing mortality, and predation mortality using the CEATTLE model in `TMB`. To run `Rceattle` a user can either load a pre-build Bering Sea data set or build a data set from an excel spreadsheet. The model will then initialize assuming $0$ for all parameters unless a parameter list is provided from a previous model. Documentation for all functions can be found using `?`.
