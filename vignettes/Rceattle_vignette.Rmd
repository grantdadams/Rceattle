...
bibliography:
csl: https://github.com/citation-style-language/styles/blob/master/dependent/fisheries-research.csl
---
...
parameters_table <- function(){
  ...
  Parameters <- rbind(Parameters, c("Annual recruitment deviation",  "$\\tau_{i,y}$",  "E", ""))
  ...
  Parameters <- rbind(Parameters, c("Observed BT survey biomass (kg)",  "$\\beta^s_{i,y}$",  "D", "srv_biom"))
  Parameters <- rbind(Parameters, c("Observed EIT survey biomass (kg)",  "$\\beta^{eit}_y$",  "D", "obs_eit"))
  Parameters <- rbind(Parameters, c("Bottom temperature (°C)",  "$T_y$",  "D", ""))
  Parameters <- rbind(Parameters, c("Gravimetric proportion of prey in predator stomach",  "$U_{paij,y}$",  "D", ""))
  Parameters <- rbind(Parameters, c("Biomass of other prey (kg)",  "$B^{other}_p$",  "D", ""))
  ...
  #
  params_extra <- rbind(params_extra, c("Total catch", "$C_{i,y}$", "D", "tc_obs"))
  ...
  knitr::kable(Parameters[,1:4], escape = T, format = "html", warning = FALSE, message = FALSE)
}
parameters_table()
```
...
Equation <- c(
  "",
  "$\\sum_i \\sum_y \\frac{\\left[ ln \\left(\\beta^s_{i,y} \\right) - ln \\left(\\hat{\\beta}^s_{i,y} \\right) \\right]} {2 \\sigma^2_{s,i}}$",
  "$-\\sum_i n_i \\sum_y \\sum_j \\left( O^s_{ij,y} + v \\right) ~ ln \\left(\\hat{O}^s_{ij,y} + v \\right)$",
  "$\\sum_i \\sum_y \\frac{\\left[ ln \\left(\\beta^{eit}_{y} \\right) - ln \\left(\\hat{\\beta}^{eit}_{y} \\right) \\right]} {2 \\sigma^2_{eit}},~\\sigma_{eit}=0.2$",
  "$- n \\sum_y \\sum_j \\left( O^{eit}_{1j,y} + v \\right) ~ ln \\left(\\hat{O}^{eit}_{1j,y} + v \\right)$",
  "$\\sum_i \\sum_y \\frac{\\left[ ln \\left(C^*_{i,y} \\right) - ln \\left(\\hat{C}^*_{i,y} \\right) \\right]} {2 \\sigma^2_{C}},~\\sigma_C = 0.05$",
...
knitr::kable(Likelihood_Components, escape = T, format = "html")
```
