...
Skip this if no environmental covariates are used. `clean_data()` will fall back to a Year-only data.frame with no indices, and validation only complains when a feature actually needs an index (env-q catchability, `Ceq[sp] > 1`, or any env linkage).
...
These parameters govern the Wisconsin bioenergetics model used to compute predation mortality (M2) in multi-species mode. They are unused in single-species mode and can be omitted entirely — Rceattle fills any missing scalars with safe defaults.
