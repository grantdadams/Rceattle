---
title: "6. Building a data object without Excel"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteIndexEntry{6. Building a data object without Excel}
...
simData$minage  <- rep(1, nspp)       # minimum age (usually 1 or 0)
...
# Used internally to convert length-based selectivity to weight.
simData$alpha_wt_len <- rep(0.00001, nspp)
simData$beta_wt_len  <- rep(3, nspp)

# Prior standard deviation for log-recruitment deviations. Controls the
...
#   NA                         -- not applicable (use for fishery fleets)
...
simData$fleet_control <- data.frame(
  ...
  Sel_curve_pen2            = NA,
  # Time_varying_sel:
  #   0 = time-invariant (default)
  #   1 = penalized log-normal deviates (SD fixed at Time_varying_sel_sd_prior
  #       unless random_sel = TRUE in fit_mod)
  #   3 = time blocks (specify via R-side mapping)
  #   4 = random walk
  Time_varying_sel          = 0,
  ...
  # Time_varying_q:
  #   0 = time-invariant
  #   1 = penalized log-normal deviates
  #   3 = time blocks
  #   4 = random walk
...
An optional time series used in environment-recruitment, -M1, or -catchability relationships. Missing years are filled with the column mean. Column names after `Year` can be anything and are referenced by column order.
...
## 8. Multi-species bioenergetics parameters (optional)

These parameters govern the Wisconsin bioenergetics model used to compute predation mortality (M2) in multi-species mode. They are unused in single-species mode. See `?build_M1` and the CEATTLE technical documentation for full details on each parameter.
...
# Ration data (optional): observed mean ration-at-age from bioenergetics studies.
# Leave empty if not available.
simData$ration_data <- data.frame(matrix(NA, nrow = 0, ncol = 3 + nages))
...
# Leave empty if not available — the model will use the empirical suitability
# approach (suitMode = 0) derived from whatever diet data are provided.
simData$diet_data <- data.frame(matrix(NA, nrow = 0, ncol = 9))
...
`data_check()` prints a summary of the data list and flags common issues (mismatched dimensions, missing columns, impossible values) before any model fitting is attempted.
