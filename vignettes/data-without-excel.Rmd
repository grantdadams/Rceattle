...
The list below uses every field the model knows about, but most of them are optional and only needed when a particular feature is turned on. `clean_data()` fills missing optional fields with safe defaults, and `data_check()` enforces them only under the conditions that actually require them:
...
| `env_data`                               | env-q catchability, `Ceq[sp] > 1`, env linkages, `srr_indices` / `M1_indices` non-empty |
| `diet_data`                              | `msmMode > 0`                                  |
| `ration_data`                            | `msmMode > 0`                                  |
| Bioenergetics scalars (`Ceq`, `CA`, `CB`, `Tco`, ...) | `msmMode > 0`                                  |

In single-species mode (`msmMode = 0`) you can omit any of these by simply not setting the field on the list. The single-species example below shows the full machinery, but feel free to skip sections you don't need.
