...
|---|---|---|---|---|---|---|
| `Sel_curve_pen1` | `Sel_shape_sd` | only on 9 under `Sel_shape_mode = "Directional"` | shape, on the base curve | shape, on the base curve | shape, on the base curve | random walk on realized log-selectivity |
| `Sel_curve_pen2` | `Sel_curvature_sd` | no | 2nd-difference curvature | 2nd-difference curvature | 2nd-difference curvature | not read |
...
- **The sign.** A weight multiplies a squared deviation, so a negative one
  *rewards* that deviation. `data_check()` refuses a negative `Sel_curve_pen`
  wherever the template reads it as a weight, the directional case above being
  the one exemption.
- **Whether the deviates exist.** The three slots marked as random walks or
  deviation magnitude are charged on the time-varying deviates, so under
...
correlations, not weights, and keep those columns.
...
See **Selectivity penalty weights** below for `Sel_curve_pen1/2/3` and the
