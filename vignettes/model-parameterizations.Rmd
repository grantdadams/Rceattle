...
The `Sel_curve_pen1/2/3` are penalty *weights*. Each penalty is a Gaussian sum-of-squares ($\text{weight}\cdot x^2 = x^2 / (2\sigma^2)$), so you may instead supply them as **standard deviations**: `Sel_shape_sd` (with `Sel_shape_dir = "Decreasing"`/`"Increasing"` for the directional sign), `Sel_curvature_sd`, and `Sel_devmag_sd`, which are converted to the weights via $\text{weight} = 1/(2\sigma^2)$. Supplying an SD sets the matching `Sel_curve_pen` column; a `Sel_curve_pen` value you provide directly is never overwritten. `Sel_shape_sd` applies to `NonParametric` (2), `NonParametricPM` (9), `NonParametricIntegrable` (13) and `LogisticPM` (11); `Sel_curvature_sd` to the three non-parametric forms (LogisticPM does not use `Sel_curve_pen2`); `Sel_devmag_sd` only to `NonParametricPM` (9) and `LogisticPM` (11), the slot being unread by 2 and 13. Only `NonParametricPM` (9) with `Sel_shape_mode = "Directional"` branches on `Sel_shape_dir`; every other form -- that one under `"Smooth"` included, where the term is two-sided -- applies `Sel_curve_pen1` as written, so a negative weight there would reward a decreasing curve rather than penalize it and `Sel_shape_dir = "Increasing"` is refused. For the same reason `data_check()` refuses a negative `Sel_curve_pen` written directly on a form that reads it as a weight. The 2D/3D-AR1 forms reuse `Sel_curve_pen` for logit-scale correlations and keep those columns.
