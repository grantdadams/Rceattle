...
**With `$ARGUMENTS`** → resolve it to a file under `tests/testthat/` (accept a partial
name) and run just that file with the shared helpers loaded — the project idiom:
...
export PATH=/usr/bin:$PATH && NOT_CRAN=true Rscript -e '
  ...
  testthat::test_file("tests/testthat/<resolved-path>", env = e, reporter = "progress")'
```
