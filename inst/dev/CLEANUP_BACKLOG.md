...
| `R/1-data_check.R` (`est_sel_flts <- ...`, the estimated-selectivity subset) | **Open** (found reviewing the 5.34.0-5.42.1 release, #158). An `NA` `Fleet_type` on any fleet | `data_check()` dies with the anonymous `missing value where TRUE/FALSE needed` instead of naming the fleet or the column. The subset compares `Fleet_type != "Off"`, which is `NA` rather than `FALSE`, and an `NA` logical subscript keeps an all-`NA` ROW: measured on `Atka2022` with `Fleet_type[1] <- NA`, `est_sel_flts` has 2 rows with `Fleet_code` `NA, 2`. The error is then raised inside the `vapply` over those rows, at an `||` on an `NA`, not at the `any()` below it. `switch_check()` passes the `NA` through unchanged and nothing refuses it by name, while `build_map_selectivity()` deliberately tolerates one (`.on[is.na(.on)] <- TRUE`, treating it as estimated) -- so the two halves of the package disagree about whether the state is legal. Decide that first: refuse `NA` `Fleet_type` in `switch_check()`, or normalize it to the estimated case everywhere with the `%in%` idiom. **A related pre-existing hole found in the same pass and NOT fixed either:** an integer-coded `Fleet_type` (`0` for Off, as a pre-`switch_check()` workbook stores it) reads as LIVE at this and every other `!= "Off"` site, because `0 != "Off"` is `TRUE`. Not fixed in the release PR: both change `data_check()` semantics on paths that error or mis-select today, and the release was days from a tag. |
