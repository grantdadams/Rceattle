...
|---|---|---|
| `R/3-build_map.R` (`# Age-independent scalar`) | **Open.** `estDynamics = 3` in a multispecies model (`msmMode > 0`) | The branch is gated on `estDynamics[sp] == 2 \| msmMode != 0`. Under `msmMode > 0` it maps out every age after `minage` (columns 2..`nages` of `log_pop_scalar`) for every species, `estDynamics = 3` included. So the age-specific scalar is never estimated: case 3 in the template multiplies those ages by `exp(init)`, which is 1 unless `inits` are supplied. `estDynamics[sp] == 2` alone (or `&`) is safe for codes 0-2. Before changing it, reproduce with a Hessian check, since the freed scalars are informed only through predation and that species' index. The `# Age-dependent scalar` branch has the same `\|`, but it only maps padding columns beyond `nages`, so it is harmless. Separately, `# Don't estimate the scalar` maps every column out under `msmMode = 0`, so `estDynamics` 2 and 3 behave as 1 in single-species models. Neither the schema nor `?BS2017SS` says so, though the 5.30.0 NEWS does; this is a question for Grant. Found reviewing PR #143 (2026-09-12); not yet reproduced with a fit. |
| `JNLL_Q_PRIOR` against a q linkage intercept prior (`linkage_is_intercept`) | **Open.** `Catchability = "Estimated-with-prior"` plus a q linkage `(Intercept)` prior on the same fleet | Both score that fleet's `index_log_q`. A lognormal linkage prior combines with the q prior into one lognormal with precision `1/s1^2 + 1/s2^2`: the SD is `s/sqrt(2)` when the two match. A normal, gamma or beta linkage prior is a second, different density on the natural scale. The two land in different `jnll_comp` rows, so a likelihood table shows both. `.check_q_linkage_support()` (run from `fit_mod()`) refuses only self-built and unestimated forms, and its message recommends `"Estimated-with-prior"`. That function is where a guard belongs; 5.33.0's `build_srr()` refuses the Ricker-alpha form of this pair with `.has_intercept_prior()`. Found reviewing PR #143; not yet reproduced with a fit. |
| `JNLL_M_PRIOR` against an M1 linkage intercept prior (`linkage_is_intercept`) | **Open.** `M1_use_prior = TRUE` with `M2_use_prior = FALSE`, plus an M1 linkage `(Intercept)` prior | The M prior scores the `log_M1` cells `M1_model` estimates (one per species, per sex, or per sex and age); the intercept prior scores the cells its rows cover, so those cells are counted twice. That is the whole prior only under `M1_model = 1`. The combination works as for q, above. `build_M1()` receives both settings and has no guard; it can refuse the pair the way `build_srr()` does. Found reviewing PR #143; not yet reproduced with a fit. |
| `srr_terms_on` | **Open, low.** A recruitment linkage intercept prior on an `estDynamics > 0` species | The gate covers the stock-recruit prior and the curve penalty, not the linkage-prior loop. `rec_pars` is mapped out for such a species, so the prior only adds a constant. It moves the objective and `JNLL_LINKAGE_PRIOR` (likelihood tables, AIC), but no estimate. Not yet checked: slope rows on such a species, which `build_map_fixed_natage()` does not map out. Found reviewing PR #143. |
| `// Input SB0 (if running in multi-species mode)` | **Open.** `msmMode = 0` with `estDynamics > 0` | Equilibrium `SB0`/`SBF` are still built on placeholder recruitment; from 5.30.0, only the dynamic runs keep the input numbers. HCRs 5, 6 and 7 read `SB0` when `DynamicHCR = FALSE`, and 5 also reads `SBF`. With `DynamicHCR = FALSE`, `ssb_depletion` and `biomass_depletion` divide by `SB0` and `B0` under every HCR. Projected numbers come from `NByageFixed`, so the dynamics are right. What is wrong is the reported depletion, and F and catch advice under those HCRs. Found reviewing PR #143; not yet reproduced with a fit. |
| `int spawn_yr = yr - minage(sp);`, `int rp_yr = yr - minage(sp);` | **Open.** `minage = 0` with a stock-recruit curve | Each lag reads the same year's `ssb`/`SB0`/`DynamicSB0` before it is accumulated, so it reads 0 and the curve gives R = 0. This happens in three places, each under its own switch: the hindcast under `srr_fun >= 2`; the reference-point runs under `srr_pred_fun >= 2`, the Ianelli form included; and the projection under `proj_mean_rec = 0`. Under the Ianelli form the hindcast is unaffected, and `SB0` decays with no error. `data_check()` refuses only `minage < 0`. The GOA and AI cod `minage = 0` models fit no curve. Found reviewing PR #143; not yet reproduced with a fit. |
