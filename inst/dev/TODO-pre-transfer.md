...
- `dev` is at 5.41.0. `main` is at 5.33.0, 76 commits behind `dev`.
...
- [ ] **B2. Ship the 5.34.0–5.41.0 release [either, Grant publishes]**, following
  `inst/RELEASE-CHECKLIST.md` exactly:
  1. Open and merge the `dev` -> `main` PR.
  2. Tag **the merge commit** with the bare version (`5.41.0`, **no `v`**; checklist section 3).
...
- [ ] **B3. Make `golden` robust [agent, Grant reviews].**
  ...
  Ship it as a patch release (5.41.1) if it lands after B2, so the version being transferred
...
- [x] **B4. Apply the tag convention everywhere [agent].** Already satisfied, verified
  ...
  `README.md:43` reads `@5.41.0`. Both are bare, per the convention.
- [x] **B5. Check the other install lines [agent].** Verified with `git grep -n
  ...
  files is either untagged or uses the bare convention. `README.md:43` names `5.41.0`, which
