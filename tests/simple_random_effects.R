...
# Plot function
plot_fun <- function(obs, est, CI, x, obs_var, est_var, var_CI, file_name = NULL){

  tiff( file = paste0("tests/",file_name,".tiff") , width=169 / 25.4, height = 100 / 25.4, family = "serif", units = "in", res = 100)
  ...
  # Plot variance
  min_y <- min( c(0, est_var - var_CI, est_var + var_CI, obs_var), na.rm = T)
  max_y <- max( c(0, est_var - var_CI, est_var + var_CI, obs_var), na.rm = T) + .75
  plot(NA, NA, xlab = NA, ylab = "Variance", ylim = c(min_y, max_y), xlim = c(0.35, 0.75), xaxt = "n")
