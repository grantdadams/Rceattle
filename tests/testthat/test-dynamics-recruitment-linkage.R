...
testthat::test_that("Beverton alpha-linked recruitment", {
  ...
  expect_all_true(!is.na(c(ss_run$map$mapList$rec_pars[, 2:3])))
  expect_all_true(!is.na(c(ss_run$map$mapList$beta_linkage[
  ...
  expect_all_true(sp3_offset == 0) # Species3
})
...
testthat::test_that("Beverton alpha-linked recruitment (penalty approach)", {
  ...
  expect_all_true(!is.na(c(ss_run$map$mapList$rec_pars[, 1])))
  expect_all_true(!is.na(c(ss_run$map$mapList$rec_pars[, 2:3])))
  expect_all_true(!is.na(c(ss_run$map$mapList$beta_linkage[
    ss_run$data_list$linkage_table$design_col != "(Intercept)"])))
  expect_all_true(is.na(c(ss_run$map$mapList$beta_linkage[
