testthat::test_that("Rceattle and multi-species model dynamics match", {
  ...
  ms_fixed_model <- Rceattle::fit_mod(data_list = simData,
  ...
  # Recruitment
  testthat::expect_equal(as.numeric(sim$model_quantities$NAA[,1,]), as.numeric(ms_fixed_model$quantities$R[,1:nyrs]))
  testthat::expect_equal(as.numeric(sim$model_quantities$Total_Biom), as.numeric(ms_fixed_model$quantities$biomass[,1:nyrs]), tolerance = 1e-6)


  # Suitability
  testthat::expect_equal(exp(ms_fixed_model$estimated_params$log_gam_a), gam_a)
  testthat::expect_equal(exp(ms_fixed_model$estimated_params$log_gam_b), gam_b)
  testthat::expect_equal(as.numeric(ms_fixed_model$quantities$vulnerability), as.numeric(sim$model_quantities$vulnerability))
  testthat::expect_equal(as.numeric(ms_fixed_model$quantities$suitability[,,,,1]), as.numeric(sim$model_quantities$suitability))
  testthat::expect_equal(as.numeric(ms_fixed_model$quantities$suit_other[,1,1,1]), as.numeric(sim$model_quantities$suit_other))

  # M2
  testthat::expect_equal(as.numeric(sim$model_quantities$M2_at_age), as.numeric(ms_fixed_model$quantities$M2_at_age[,1,,1:nyrs]), tolerance = 1e-6)

  # Ration
  testthat::expect_equal(as.numeric(sim$model_quantities$ration), as.numeric(ms_fixed_model$quantities$ration[,1,,1]))

  # N
  testthat::expect_equal(as.numeric(sim$model_quantities$NAA[,,]), as.numeric(ms_fixed_model$quantities$N_at_age[,1,,1:nyrs]))

  # AvgN
  testthat::expect_equal(as.numeric(sim$model_quantities$avgNAA[,,]), as.numeric(ms_fixed_model$quantities$avgN_at_age[,1,,1:nyrs]))

  # Avail food
  testthat::expect_equal(as.numeric(sim$model_quantities$avail_food[,,1]), as.numeric(ms_fixed_model$quantities$avail_food[,1,,1]))

  # Selectivity
  testthat::expect_equal(as.numeric(sim$model_quantities$srv_sel[,]), as.numeric(ms_fixed_model$quantities$sel[c(1,3),,,1]))
  testthat::expect_equal(as.numeric(sim$model_quantities$fish_sel[,]), as.numeric(ms_fixed_model$quantities$sel[c(2,4),,,1]))

  # F
  testthat::expect_equal(as.numeric(sim$model_quantities$FAA), as.numeric(ms_fixed_model$quantities$F_flt_age[c(2,4),1,,1:nyrs]))

  # Q
  testthat::expect_equal(as.numeric(sim$model_quantities$srv_q), as.numeric(ms_fixed_model$quantities$index_q[c(1,3),1]))

  # Expected and observed diet
  testthat::expect_equal(as.numeric(ms_fixed_model$data_list$diet_data$Stomach_proportion_by_weight), as.numeric(ms_fixed_model$quantities$diet_hat[,2]))
})
...
testthat::test_that("Test proportion of prey-at-age in predator-at-age averaged across years", {
  ...
  diet_data1 <- ms_fixed_model$data_list$diet_data %>%
...
testthat::test_that("Test annual proportion of prey (all ages) in predator (weighted mean across all ages)", {
  ...
  avgN <- sim$model_quantities$NAA # Dims: [pred, pred_age, year]
