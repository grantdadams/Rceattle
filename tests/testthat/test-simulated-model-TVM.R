testthat::test_that("Rceattle and multi-species model dynamics match", {
  ...
  # M
  testthat::expect_equal(as.numeric(sim$model_quantities$MAA), as.numeric(ms_run1$quantities$M_at_age[,1,,1:nyrs]), tolerance = 1e-6)

  # M map
  testthat::expect_equal(matrix(ms_run1$map$mapList$ln_M1_dev[1,1,,], 15, 40), matrix(c(1:30, rep(NA, 10)), 15, 40, byrow = TRUE))
  testthat::expect_equal(matrix(ms_run1$map$mapList$ln_M1_dev[2,1,,], 15, 40), matrix(c(31:60, rep(NA, 10)), 15, 40, byrow = TRUE))

  # N
  testthat::expect_equal(as.numeric(sim$model_quantities$NAA[,,]), as.numeric(ms_run1$quantities$N_at_age[,1,,1:nyrs]))

  # Likelihood
  expected_nll <- -sum(dnorm(sim$model_quantities$Mtv, 0, 0.1, TRUE))
  obs_nll <- sum(ms_run1$quantities$jnll_comp[16,])
