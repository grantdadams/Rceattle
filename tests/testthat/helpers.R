...
make_test_data <- function(nyrs = 8, nages = 5, seed = NULL) {
  ...
  simData$fleet_control <- data.frame(
    ...
    Fleet_code = 1:2,
    Fleet_type = 2:1, # Fleet type; 0 = don't fit, 1 = fishery, 2 = survey
    Species = 1,
...
make_msm_test_data <- function(
  ...
  simData$fleet_control <- data.frame(
    ...
    Fleet_code = 1:(nspp * 2),
    Fleet_type = rep(2:1, nspp), # Fleet type; 0 = don't fit, 1 = fishery, 2 = survey
  ...
  simData$Pyrs <- do.call(rbind, ration_list)
