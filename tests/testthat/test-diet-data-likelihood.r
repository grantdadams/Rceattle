...
gam_a = c(1, 0.1)
gam_b = rep(0.15, nspp)
log_phi = matrix(c(-2,0.5,-4,-2), nspp, nspp, byrow = TRUE) #matrix(c(-5,0.5,-10,-2), nspp, nspp, byrow = TRUE)
other_food <- rep(1e5, nspp)
...
ms_run0 <- Rceattle::fit_mod(data_list = simData,
...
plot(x = sim$NAA[1,1,], y = ms_run0$quantities$R[1,1:nyrs]); abline(1,1)
plot(x = sim$NAA[2,1,], y = ms_run0$quantities$R[2,1:nyrs]); abline(1,1)

plot(x = sim$Total_Biom[1,], y = ms_run0$quantities$biomass[1,1:nyrs]); abline(1,1)
plot(x = sim$Total_Biom[2,], y = ms_run0$quantities$biomass[2,1:nyrs]); abline(1,1)

# Suitability is OK
exp(ms_run0$estimated_params$log_gam_a)-gam_a
exp(ms_run0$estimated_params$log_gam_b)-gam_b

sum(ms_run0$quantities$suitability[,,,,1] - sim$suitability)
ms_run0$quantities$vulnerability-sim$vulnerability

sim$suit_other -  ms_run0$quantities$vulnerability_other

# M2
sum(sim$M2_at_age - ms_run0$quantities$M2_at_age[,1,,1:nyrs])

# Ration
ms_run0$quantities$ration[,1,,1] - sim$ration

# AvgN
sum(ms_run0$quantities$avgN_at_age[,1,,1:nyrs] -sim$avgNAA[,,])

# Avail food
ms_run0$quantities$avail_food[,1,,1]-sim$avail_food[,,1]

# Selectivity
ms_run0$quantities$sel_at_age[c(1,3),,,1]-sim$srv_sel
ms_run0$quantities$sel_at_age[c(2,4),,,1]-sim$fish_sel

# F
sum(ms_run0$quantities$F_flt_age[c(2,4),1,,1:nyrs] - sim$FAA)

# Q
ms_run0$quantities$index_q
sim$srv_q

#plot_diet_comp(ms_run0)

ms_run0$quantities$suitability[2,1,1:5 ,1:5 ,1]
ms_run0$quantities$suit_other[2,1,1:5 ,1]
ms_run0$quantities$avail_food[2,1,1:5 ,1]
ms_run0$quantities$vulnerability
ms_run0$quantities$vulnerability_other
ms_run0$data_list$other_food
ms_run0$quantities$diet_prop_hat[1,1,1:5, 1:5, 1]
ms_run0$quantities$diet_prop_hat[1,2,1:5, 1:5, 1]


##################################################################
## Test Case: Disaggregated Diet Data ----
##################################################################
...
##################################################################
## Test Case: Diet Data Averaged Across Years ----
##################################################################
...
#plot_diet_comp(ms_run2)

##################################################################
## Test Case: Prey-Age Aggregated Data ----
##################################################################
...
plot(
  ...
  xlab = "True Diet Proportion (Simulated)",
  ylab = "Est Diet Proportion (Modeled)",
  main = "Fit for Prey-summed Annual Diet Data"
)
abline(0, 1)


###############################################################
## Test Case: Prey-Age AND Year Aggregated Data ----
###############################################################
...
#plot_diet_comp(ms_run4)


###############################################################
## Test Case: Predator-Age AND Prey-Age Aggregated ----
###############################################################
...
###############################################################
## Test Case: All Aggregations (Pred-Age, Prey-Age, Year) ----
###############################################################
