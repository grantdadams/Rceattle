...
Type objective_function<Type>::operator() (){
  ...
  for(i=0; i < nspp; i++){
    for(j=0; j < nages(i); j++){
      ...
      if(j == (nages(i)-1)){
        NByage(0, j, i) = exp(ln_mn_rec(i) - (j+1)*M1(i,j) + init_dev(i, j))/ (1-exp(-M1(i, nages(i)-1))); // NOTE: This solves for the geometric series
      }
    }
  }
  REPORT( M1 );
  ...
  for(j=0; j < nages(0); j++){
    for(y=0; y < n_eit; y++){
      ...
      eit_age_hat(j, y) = NByage(eit_yr_ind+1, j, 0) * exp(-Zed(eit_yr_ind, j, 0)) * eit_sel(j, y) * eit_q; // Remove the mid-year trawl?
  ...
  for(i=0; i < nspp; i++){
    for(j=0; j < nages(i); j++){
      for(y=0; y < nyrs_srv_biom(i); y++){
        ...
        srv_bio_hat(i, y) += srv_age_hat(y, j, i) * wt(i, j, srv_yr_ind);  //
      }
    }
  }
