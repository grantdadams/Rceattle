...
Type objective_function<Type>::operator() () {
  ...
  DATA_VECTOR(MSSB0); DATA_UPDATE(MSSB0); // SB0 from projecting the model forward in multi-species mode under no fishing
  DATA_VECTOR( MSB0 ); DATA_UPDATE(MSB0); // B0 from projecting the model forward in multi-species mode under no fishing
 ...
 */
  ...
  for (sp = 0; sp < nspp; sp++) {
    ...
    if(forecast == 1 & msmMode > 0){
      ...
      // -- Avg F (have F limit)
      if(HCR == 2){
        jnll_comp(13, sp)  += square((biomassSSB(sp, nyrs-1)/SB0(sp, nyrs-1))-FsprLimit(sp)) * SB0(sp, nyrs-1)/1000;
      }

      // F that achieves \code{Ftarget}% of SSB0 in the end of the projection
      if(HCR == 3){
        jnll_comp(13, sp)  += square((biomassSSB(sp, nyrs-1)/SB0(sp, nyrs-1))-FsprTarget(sp)) * SB0(sp, nyrs-1)/1000;
      }
