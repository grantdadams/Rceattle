...
Type objective_function<Type>::operator() () {
  ...
  int max_nages = imax(nages);              // Integer of maximum nages to make the arrays; n = [1]
  ...
  DATA_MATRIX( stom_prop_obs );               // pred, prey, predA, preyA U observations (mean wt_hat of prey in each pred age); n = [nspp, nspp, max_nages, max_nages]
  DATA_IMATRIX( stom_prop_ctl );          // Info on pred, prey, predA, preyA U matrix (mean wt_hat of prey in each pred age); n = [nspp, nspp, max_nages, max_nages]
  ...
  // 4.1. Derived indices
  int max_nlengths = imax( nlengths );                                                   // Integer of maximum number of length/age bins.
  ...
  // array<Type>   biomass_by_age(nspp, 2, max_nages, nyrs); biomass_by_age.setZero();// Estimated biomass-at-age (kg)
  ...
  // array<Type>   ssb_by_age(nspp, max_nages, nyrs); ssb_by_age.setZero();           // Spawning biomass at age (kg)
  array<Type>   M_at_age(nspp, 2, max_nages, nyrs); M_at_age.setZero();               // Total natural mortality at age
  array<Type>   M1_at_age(ln_M1.dim); M1_at_age = exp(ln_M1);                       // Residual or total natural mortality at age
  array<Type>   N_at_age(nspp, 2, max_nages, nyrs); N_at_age.setZero();               // Numbers at age
  array<Type>   avgN_at_age(nspp, 2, max_nages, nyrs); avgN_at_age.setZero();         // Average numbers-at-age
  array<Type>   sex_ratio_hat(nspp, max_nages, nyrs); sex_ratio_hat.setZero();        // Estimated age-specific sex ratin
  matrix<Type>  avg_sex_ratio_hat(nspp, nyrs); avg_sex_ratio_hat.setZero();         // Estimated sex ratio across all ages
  // array<Type>   S_at_age(nspp, 2, max_nages, nyrs); S_at_age.setZero();            // Survival at age
  array<Type>   Z_at_age(nspp, 2, max_nages, nyrs); Z_at_age.setZero();               // Total mortality at age
  ...
  array<Type>   sel(n_flt, 2, max_nages, nyrs); sel.setZero();                        // Estimated selectivity at age
  matrix<Type>  avg_sel(n_flt, 2); avg_sel.setZero();                               // Average selectivity
  array<Type>   non_par_sel(n_flt, 2, max_nages); non_par_sel.setZero();              // Temporary saved selectivity at age for estimated bits
  ...
  array<Type>   F_flt_age(n_flt, 2, max_nages, nyrs); F_flt_age.setZero();            // Estimated fishing mortality-at-age/sex for each fishery
  array<Type>   Flimit_age_spp(nspp, 2, max_nages, nyrs); Flimit_age_spp.setZero();   // Estimated target fishing mortality-at-age/sex for each species
  array<Type>   Ftarget_age_spp(nspp, 2, max_nages, nyrs); Ftarget_age_spp.setZero(); // Estimated limit fishing mortality-at-age/sex for each species
  array<Type>   F_spp_at_age(nspp, 2, max_nages, nyrs+1); F_spp_at_age.setZero();     // Sum of annual estimated fishing mortalities for each species-at-age
  ...
  array<Type>   NByage0(nspp, 2, max_nages, nyrs); NByage0.setZero();                 // Numbers at age at mean recruitment and F = 0
  array<Type>   NByageF(nspp, 2, max_nages, nyrs); NByageF.setZero();                 // Numbers at age at mean recruitment and F = Flimit
  array<Type>   DynamicNByage0(nspp, 2, max_nages, nyrs); DynamicNByage0.setZero();   // Numbers at age at F = 0 (accounts for annual recruitment)
  array<Type>   DynamicNByageF(nspp, 2, max_nages, nyrs); DynamicNByageF.setZero();   // Female numbers at age at F = Ftarget (accounts for annual recruitment)
  ...
  array<Type>   NbyageSPR(4, nspp, max_nages);                                        // Estimated numbers at age for spawning biomass per recruit reference points
  ...
  array<Type>   consumption_at_age( nspp, 2, max_nages, nyrs ); consumption_at_age.setZero();           // Pre-allocated indiviudal consumption in grams per predator-age
  matrix<Type>  fT( nspp, nyrs ); fT.setZero();                                     // Pre-allocation of temperature function of consumption
  // matrix<Type>  mnWt_obs( nspp, max_nages ); mnWt_obs.setZero();                   // Mean observed weight at age (across years)
  array<Type>   ration( nspp, 2, max_nages, nyrs ); ration.setZero();                 // Annual ration at age (kg/yr)

  // -- 4.9. Diet components
  array<Type>   diet_prop(nspp * 2, nspp * 2, max_nages, max_nages, nyrs); diet_prop.setZero();             // Stomach proportion by weight U
  //array<Type>   diet_prop_hat(nspp * 2, nspp * 2, max_nages, max_nages, nyrs); diet_prop_hat.setZero();   // Predicted stomach proportion by weight U
  //array<Type>   avg_diet_prop_hat(nspp * 2, nspp * 2, max_nages, max_nages); avg_diet_prop_hat.setZero(); // Average predicted stomach proportion by weight across years U
  array<Type>   other_food_diet_prop(nspp, 2, max_nages, nyrs); other_food_diet_prop.setZero();           // Other food diet proportion by weight
  ...
  array<Type>   avail_food(nspp, 2, max_nages, nyrs); avail_food.setZero();                                           // Available food to predator
  array<Type>   stom_div_bio(nspp * 2, nspp * 2, max_nages, max_nages, nyrs); stom_div_bio.setZero();                   // Stomach proportion over biomass; U/ (W * N)
  array<Type>   suit_main(nspp * 2, nspp * 2, max_nages, max_nages, nyrs); suit_main.setZero();                         // Suitability/gamma selectivity of predator age u on prey age a
  array<Type>   suit_other(nspp, 2, max_nages, nyrs); suit_other.setZero();                                           // Suitability not accounted for by the included prey
  array<Type>   suma_suit(nspp, 2, max_nages, nyrs); suma_suit.setZero();                                             // Sum of suitabilities
  ...
  array<Type>   M2_at_age(nspp, 2, max_nages, nyrs); M2_at_age.setZero();                        // Predation mortality at age
  // array<Type>   M2_prop(nspp * 2, nspp * 2, max_nages, max_nages, nyrs); M2_prop.setZero();     // Relative predation mortality at age from each species at age
  array<Type>   B_eaten_as_prey(nspp, 2, max_nages, nyrs); B_eaten_as_prey.setZero();            // Biomass eaten as prey via predation
  // array<Type>   B_eaten_as_pred(nspp, 2, max_nages, nyrs); B_eaten_as_pred.setZero();         // Biomass eaten as predator via predation
  // array<Type>   B_eaten(nspp * 2, nspp * 2, max_nages, max_nages, nyrs); B_eaten.setZero();     // Biomass of prey eaten via predation by a predator at age
  // array<Type>   N_eaten(nspp * 2, nspp * 2, max_nages, max_nages, nyrs); N_eaten.setZero();     // Number of prey of age a eaten by predator age u
  ...
  /*
   ...
   array<Type>  N_pred_yrs(nspp, 2, max_nages, nyrs); N_pred_yrs.setZero();                // Effective numbers of predators for each age of prey FIXME: should be avgN_at_age?
   array<Type>  N_prey_yrs(nspp, 2, max_nages, nyrs); N_prey_yrs.setZero();                // Effective numbers of prey for each age of predator
   array<Type>  N_pred_eq(nspp, 2, max_nages); N_pred_eq.setZero();                        // Effective numbers of predators for each age of prey (styr_pred)
   array<Type>  N_prey_eq(nspp, 2, max_nages); N_prey_eq.setZero();                        // Effective numbers of prey for each age of predator

   array<Type>  pred_resp(nspp * 2, (nspp * 2)+1, max_nages, max_nages, nyrs); pred_resp.setZero();// Predator functional response +1 for other species
   array<Type>  Pred_r(nspp, 2, max_nages, nyrs); Pred_r.setZero();                          // save Pred_ratio values
   array<Type>  Prey_r(nspp, 2, max_nages, nyrs); Prey_r.setZero();                          // save Prey_ratio values

   array<Type> ration_hat(nspp, 2, max_nages, nyrs); ration_hat.setZero();                   // Annual ration by predator age each year
   array<Type> ration_hat_ave(nspp, 2, max_nages); ration_hat_ave.setZero();                 // Annual ration by predator age averaged over years
   */
      ...
      for (age = 0; age < nages(sp); age++){
        for(sex = 0; sex < nsex(sp); sex ++){
          ...
          if(initMode == 0){
            ...
            N_at_age(sp, 1, age, 0) = exp(init_dev(sp, age)) * (1.0-R_sexr(sp));
          }
          ...
          if(initMode > 0){
            ...
            if(age == 0){
              ...
              N_at_age(sp, 1, 0, 0) = R(sp, 0) * (1.0-R_sexr(sp));
            }

            // // Sum M1_at_age until age - 1
            // Type mort_sum = M1_at_age(sp, sex, 0); // skipping age-2 M_at_age
            // if(age > 1){
            ...
            if ((age > 0) & (age < nages(sp) - 1)) {
              ...
              if(sex == 1){
                N_at_age(sp, 1, age, 0) = R_init(sp) * exp( - mort_sum + init_dev(sp, age - 1)) * (1.0-R_sexr(sp));
              }
            }
            ...
            if (age == (nages(sp) - 1)) {
              ...
              if(sex == 1){
                N_at_age(sp, 1, age, 0) = R_init(sp) * exp( - mort_sum + init_dev(sp, age - 1)) / (1 - exp(-M1_at_age(sp, sex, nages(sp) - 1))) * (1.0-R_sexr(sp));
              }
            }
          }
        ...
        default:
          ...
          /*
           biomass_by_age(sp, sex, age, 0) = N_at_age(sp, sex, age, 0) * wt( pop_wt_index(sp), sex, age, 0);
           biomass(sp, 0) += biomass_by_age(sp, sex, age, 0);
           */
          biomass(sp, 0) += N_at_age(sp, sex, age, 0) * wt( pop_wt_index(sp), sex, age, 0);
        }

        // -- 6.5.4. Estimated initial female ssb
        /*
         ssb_by_age(sp, age, 0) = N_at_age(sp, 0, age, 0) * pow(S_at_age(sp, 0, age, 0), spawn_month(sp)/12.0) * wt(ssb_wt_index(sp), 0, age, 0 ) * pmature(sp, age); // 6.6.
         ssb(sp, 0) += ssb_by_age(sp, age, 0);
         */

        ssb(sp, 0) += N_at_age(sp, 0, age, 0) * pow( exp(-Z_at_age(sp, 0, age, 0)), spawn_month(sp)/12.0) * wt(ssb_wt_index(sp), 0, age, 0 ) * pmature(sp, age); // 6.6.
      }
    }
      ...
      for (yr = 1; yr < nyrs_hind; yr++) {
        ...
        for (age = 0; age < nages(sp); age++){
          ...
          default:
            ...
            /*
             biomass_by_age(sp, sex, age, yr) = N_at_age(sp, sex, age, yr) * wt( pop_wt_index(sp), sex, age, yr );
             biomass(sp, yr) += biomass_by_age(sp, sex, age, yr);
             */
          ...
          /*
           ssb_by_age(sp, age, yr) = N_at_age(sp, 0, age, yr) * pow(S_at_age(sp, 0, age, yr), spawn_month(sp)/12.0) * wt(ssb_wt_index(sp), 0, age, yr ) * pmature(sp, age); // 6.6.
           ssb(sp, yr) += ssb_by_age(sp, age, yr);
           */
          ssb(sp, yr) += N_at_age(sp, 0, age, yr) * pow(exp(-Z_at_age(sp, 0, age, yr)), spawn_month(sp)/12.0) * wt(ssb_wt_index(sp), 0, age, yr ) * pmature(sp, age); // 6.6.
        }
      }
    }
      ...
      for (yr = nyrs_hind; yr < nyrs; yr++){
        ...
        for (age = 0; age < nages(sp); age++) {
          ...
          default: // Wrong estDynamics
            ...
            /*
             biomass_by_age(sp, sex, age, yr) = N_at_age(sp, sex, age, yr) * wt( pop_wt_index(sp), sex, age, nyrs_hind-1 ); // 6.5.
             biomass(sp, yr) += biomass_by_age(sp, sex, age, yr);
             */
          ...
          /*
           ssb_by_age(sp, age, yr) = N_at_age(sp, 0, age, yr) * pow(S_at_age(sp, 0, age, yr), spawn_month(sp)/12.0) * wt(ssb_wt_index(sp), 0, age, nyrs_hind-1 ) * pmature(sp, age); // 6.6.
           ssb(sp, yr) += ssb_by_age(sp, age, yr);
           */
          ssb(sp, yr) += N_at_age(sp, 0, age, yr) * pow(exp(-Z_at_age(sp, 0, age, yr)), spawn_month(sp)/12.0) * wt(ssb_wt_index(sp), 0, age, nyrs_hind-1 ) * pmature(sp, age); // 6.6.
        }
      }
    }
    ...
    for (sp = 0; sp < nspp; sp++) {
      for (yr = 0; yr < nyrs; yr++) {
        ...
        for (age = 0; age < nages(sp); age++) {
          ...
          sexr_denom += (N_at_age(sp, 0, age, yr) + N_at_age(sp, 1, age, yr)); // Males
        }
 ...
 */
    ...
    for(comp_ind = 0; comp_ind < comp_hat.rows(); comp_ind++){
      ...
      //  Catch-at-age - standardize to sum to 1
      if (comp_type == 0) {
        for (age = 0; age < nages(sp) * joint_adjust(comp_ind); age++) {
          if(n_hat(comp_ind) > 0){ //FIXME - not differentiable
