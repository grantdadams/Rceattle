...
Type objective_function<Type>::operator() () {
  ...
  # 4.1. Derived indices
  max_bin = imax( nlengths )#;;                                                   # Integer of maximum number of length/age bins.
  n_flt = nrow(fleet_control)#;;
  joint_adjust = rep(0, nrow(comp_obs))
  penalty = 0.0#;;
  ricker_intercept = 0.0#;;

  # -- 4.2. Estimated population quantities
  # pop_scalar = ln_pop_scalar#;;  pop_scalar = exp(ln_pop_scalar.array())#;;# Fixed n-at-age scaling coefficient#;; n = [nspp, nages]
  avg_R = rep(0, nspp) # -- ##;; avg_R.setZero()#;;                                       # Mean recruitment of hindcast
  R_hat = matrix(0, nrow = nspp, ncol = nyrs) # -- ##;; R_hat.setZero()#;;                                 # Expected recruitment given SR curve
  mort_sum = matrix(0, nrow = nspp, ncol = max_age) # -- ##;; mort_sum.setZero()#;;
  R0 = rep(0, nspp) # -- ##;; R0.setZero()#;;                                             # Equilibrium recruitment at F = 0.
  R_init = rep(0, nspp) # -- ##;; R_init.setZero()#;;                                     # Equilibrium recruitment at F = Finit (non-equilibrium).
  srr_alpha = 0.0#;;
  R = matrix(0, nrow = nspp, ncol = nyrs) # -- ##;; R.setZero()#;;                                         # Estimated recruitment (n)
  steepness = rep(0, nspp) # -- ##;; steepness.setZero()#;;                               # Expected % of R0 at 20% SSB0.
  # biomass_at_age = array(0, dim = c(nspp, 2, max_age, nyrs))#;; biomass_at_age.setZero()#;;# Estimated biomass-at-age (kg)
  biomass = matrix(0, nrow = nspp, ncol = nyrs) # -- ##;; biomass.setZero()#;;                             # Estimated biomass (kg)
  exploitable_biomass = matrix(0, nrow = nspp, ncol = nyrs) # -- ##;; exploitable_biomass.setZero()#;;     # Estimated exploitable biomass (kg)
  ssb = matrix(0, nrow = nspp, ncol = nyrs) # -- ##;; ssb.setZero()#;;                                     # Estimated spawning stock biomass (kg)
  biomass_depletion = matrix(0, nrow = nspp, ncol = nyrs) # -- ##;; biomass_depletion.setZero()#;;         # Estimated biomass biomass_depletion
  ssb_depletion = matrix(0, nrow = nspp, ncol = nyrs) # -- ##;; ssb_depletion.setZero()#;;                 # Estimated biomass_depletion of spawning stock biomass
  # ssb_at_age = array(0, dim = c(nspp, max_age, nyrs))#;; ssb_at_age.setZero()#;;           # Spawning biomass at age (kg)
  M_at_age = array(0, dim = c(nspp, 2, max_age, nyrs)) # -- ##;; M_at_age.setZero()#;;               # Total natural mortality at age
  M1_at_age = exp(ln_M1)#;;                       # Residual or total natural mortality at age
  N_at_age = array(0, dim = c(nspp, 2, max_age, nyrs)) # -- ##;; N_at_age.setZero()#;;               # Numbers at age
  avgN_at_age = array(0, dim = c(nspp, 2, max_age, nyrs)) # -- ##;; avgN_at_age.setZero()#;;         # Average numbers-at-age
  #sex_ratio_hat = array(0, dim = c(nspp, max_age, nyrs))#;; sex_ratio_hat.setZero()#;;        # Estimated age-specific sex ratin
  #avg_sex_ratio_hat = matrix(0, nrow = nspp, ncol = nyrs) # -- ##;; avg_sex_ratio_hat.setZero()#;;         # Estimated sex ratio across all ages
  # S_at_age = array(0, dim = c(nspp, 2, max_age, nyrs)) # -- ##;; S_at_age.setZero()#;;            # Survival at age
  Z_at_age = array(0, dim = c(nspp, 2, max_age, nyrs)) # -- ##;; Z_at_age.setZero()#;;               # Total mortality at age
  R_sd = rep(0, nspp) # -- ##;; R_sd.setZero()#;;                                         # Standard deviation of recruitment variation
  zero_N_pen = rep(0, nspp) # -- ##;; zero_N_pen.setZero()#;;                             # Additional penalty to add to likelihood if n-at-age goes < 0
  # sex_ratio_sd = rep(0, nspp) # -- ##;; sex_ratio_sd.setZero()#;;                         # Variance of sex ratio

  # -- 4.3. Selectivity parameters
  sel = array(0, dim = c(n_flt, max_age, nyrs))#;; sel.setZero()#;;                        # Estimated selectivity at age
  avg_sel = matrix(0, nrow = n_flt, ncol = 2) # -- ##;; avg_sel.setZero()#;;                               # Average selectivity
  non_par_sel = array(0, dim = c(n_flt, max_age))#;; non_par_sel.setZero()#;;              # Temporary saved selectivity at age for estimated bits
  sel_dev_sd = rep(0, n_flt)#;; sel_dev_sd.setZero()#;;                              # Standard deviation of selectivity deviates

  # -- 4.4. Fishery components
  F_spp = matrix(0, nrow = nspp, ncol = nyrs) # -- ##;; F_spp.setZero()#;;                                 # Fully selected fishing mortality by species
  F_flt = matrix(0, n_flt, nyrs)#;; F_flt.setZero()#;;                                # Fully selected fishing mortality by fleet
  F_flt_age = array(0, dim = c(n_flt, max_age, nyrs))#;; F_flt_age.setZero()#;;            # Estimated fishing mortality-at-age/sex for each fishery
  Flimit_age_spp = array(0, dim = c(nspp, 2, max_age, nyrs)) # -- ##;; Flimit_age_spp.setZero()#;;   # Estimated target fishing mortality-at-age/sex for each species
  Ftarget_age_spp = array(0, dim = c(nspp, 2, max_age, nyrs)) # -- ##;; Ftarget_age_spp.setZero()#;; # Estimated limit fishing mortality-at-age/sex for each species
  F_spp_at_age = array(0, dim = c(nspp, 2, max_age, nyrs+1))#;; F_spp_at_age.setZero()#;;     # Sum of annual estimated fishing mortalities for each species-at-age
  catch_hat = rep(0, nrow(catch_obs))#;; catch_hat.setZero()#;;                   # Estimated fishery yield/numbers (kg)
  max_catch_hat = rep(0, nrow(catch_obs))#;; max_catch_hat.setZero()#;;           # Estimated exploitable biomass/numbers by fleet (kg)
  ln_catch_sd = rep(0, nrow(catch_obs))#;; ln_catch_sd.setZero()#;;             # Estimated/fixed fishery log_sd (kg)

  # -- 4.5. Biological reference points
  NByage0 = array(0, dim = c(nspp, 2, max_age, nyrs)) # -- ##;; NByage0.setZero()#;;                 # Numbers at age at mean recruitment and F = 0
  NByageF = array(0, dim = c(nspp, 2, max_age, nyrs)) # -- ##;; NByageF.setZero()#;;                 # Numbers at age at mean recruitment and F = Flimit
  DynamicNByage0 = array(0, dim = c(nspp, 2, max_age, nyrs)) # -- ##;; DynamicNByage0.setZero()#;;   # Numbers at age at F = 0 (accounts for annual recruitment)
  DynamicNByageF = array(0, dim = c(nspp, 2, max_age, nyrs)) # -- ##;; DynamicNByageF.setZero()#;;   # Female numbers at age at F = Ftarget (accounts for annual recruitment)
  DynamicSB0 = matrix(0, nrow = nspp, ncol = nyrs) # -- ##;; DynamicSB0.setZero()#;;                       # Estimated dynamic spawning biomass at F = 0 (accounts for S_at_age-R curve)
  DynamicB0 = matrix(0, nrow = nspp, ncol = nyrs) # -- ##;; DynamicB0.setZero()#;;                         # Estimated dynamic  biomass at F = 0 (accounts for S_at_age-R curve)
  DynamicSBF = matrix(0, nrow = nspp, ncol = nyrs) # -- ##;; DynamicSBF.setZero()#;;                       # Estimated dynamic spawning biomass at F = Ftarget (accounts for S_at_age-R curve)
  NbyageSPR = array(0, dim = c(4, nspp, max_age))#;;                                        # Estimated numbers at age for spawning biomass per recruit reference points
  SPRlimit = rep(0, nspp) # -- ##;; SPRlimit.setZero()#;;                                 # Estimated Plimit SPR
  SPRtarget = rep(0, nspp) # -- ##;; SPRtarget.setZero()#;;                               # Estimated Ptarget SPR
  SPR0 = rep(0, nspp) # -- ##;; SPR0.setZero()#;;                                         # Estimated spawning biomass per recruit at F = 0
  SPRFinit = rep(0, nspp) # -- ##;; SPRFinit.setZero()#;;                                 # Estimated spawning biomass per recruit at Finit
  SB0 = matrix(0, nrow = nspp, ncol = nyrs) # -- ##;; SB0.setZero()#;;                                     # Estimated spawning stock biomass at F = 0 (Accounts for S_at_age-R)
  SBF = matrix(0, nrow = nspp, ncol = nyrs) # -- ##;; SBF.setZero()#;;                                     # Estimated spawning stock biomass at F = target (Accounts for S_at_age-R)
  B0 = matrix(0, nrow = nspp, ncol = nyrs) # -- ##;; B0.setZero()#;;                                       # Estimated biomass at F = 0 (Accounts for S_at_age-R)
  ...
  srr_mult = rep(0, ncol(beta_rec_pars))#;; srr_mult.setZero()#;;                 # Environmental design matrix for rec
  index_q_mult = rep(0, ncol(index_q_beta))#;; index_q_mult.setZero()#;;          # Environmental design matrix for q
  beta_rec_tmp = rep(0, ncol(beta_rec_pars))#;; beta_rec_tmp.setZero()#;;         # Temporary vector to store beta parameters by species for matrix mult
  env_rec_tmp = rep(0, ncol(beta_rec_pars))#;; env_rec_tmp.setZero()#;;           # Temporary vector to store env data by year for matrix mult
  beta_q_tmp = rep(0, ncol(index_q_beta))#;; beta_q_tmp.setZero()#;;              # Temporary vector to store Q beta parameters by species for matrix mult
  env_q_tmp = rep(0, ncol(index_q_beta))#;; env_q_tmp.setZero()#;;                # Temporary vector to store Q env data by year for matrix mult
  proj_F = matrix(0, nrow = nspp, ncol = nyrs) # -- ##;; proj_F.setZero()#;;                               # Projected F (Fabc/Ftac/etc) using harvest control rule


  # -- 4.6. Survey components
  index_q_sd = rep(0, n_flt)#;; index_q_sd.setZero()#;;                            # Vector of standard deviation of survey catchability prior
  index_q_dev_sd = rep(0, n_flt)#;; index_q_dev_sd.setZero()#;;                    # Vector of standard deviation of time-varying survey catchability deviation
  avgsel_tmp = 0#;;                                                              # Temporary object for average selectivity across all ages
  index_hat = rep(0, nrow(index_obs))#;; index_hat.setZero()#;;                   # Estimated survey biomass (kg)
  ln_index_sd = rep(0, nrow((index_obs))#;; ln_index_sd.setZero()#;;               # Estimated/fixed log index sd (kg)
  ln_index_analytical_sd = rep(0, n_flt)#;; ln_index_analytical_sd.setZero()#;;    # Temporary vector to save analytical sd follow Ludwig and Walters 1994
  index_q_analytical = rep(0, n_flt)#;; index_q_analytical.setZero()#;;            # Temporary vector to save analytical sd follow Ludwig and Walters 1994
  index_q = matrix(0, nrow = n_flt, ncol = nyrs_hind)#;; index_q.setZero()#;;                       # Estimated survey catchability
  index_n_obs = rep(0, n_flt)#;; index_n_obs.setZero()#;;                          # Vector to save the number of observations for each survey time series

  # -- 4.7. Composition data - FIXME: will blow up in nlengths is less than nages
  n_hat = rep(0, nrow(comp_obs)) #;; n_hat.setZero() #;;                          # Estimated catch (numbers)
  age_hat = comp_obs#;; age_hat.setZero()#;;                              # Estimated catch at true age
  true_age_comp_hat = comp_obs#;; true_age_comp_hat.setZero()#;;          # True estimated age composition
  age_obs_hat = comp_obs#;; age_obs_hat.setZero()#;;                      # Estimated catch at observed age (accounts for ageing error)
  comp_hat = comp_obs#;; comp_hat.setZero()#;;                            # Estimated comp

  # -- 4.8. Ration components
  consumption_at_age = array(0, dim = c(nspp, 2, max_age, nyrs))#;; consumption_at_age.setZero()#;;           # Pre-allocated indiviudal consumption in grams per predator-age
  fT = matrix(0, nrow = nspp, ncol = nyrs)#;; fT.setZero()#;;                                     # Pre-allocation of temperature function of consumption
  # mnWt_obs = matrix(0, nrow = nspp, ncol = max_age)#;; mnWt_obs.setZero()#;;                   # Mean observed weight at age (across years)
  ration = array(0, dim = c(nspp, 2, max_age, nyrs))#;; ration.setZero()#;;                 # Annual ration at age (kg/yr)

  # -- 4.9. Diet components
  diet_prop = array(0, dim = c(nspp * 2, nspp * 2, max_age, max_age, nyrs))#;; diet_prop.setZero()#;;             # Stomach proportion by weight U
  #diet_prop_hat = array(0, dim = c(nspp * 2, nspp * 2, max_age, max_age, nyrs))#;; diet_prop_hat.setZero()#;;   # Predicted stomach proportion by weight U
  #avg_diet_prop_hat = array(0, dim = c(nspp * 2, nspp * 2, max_age, max_age))#;; avg_diet_prop_hat.setZero()#;; # Average predicted stomach proportion by weight across years U
  other_food_diet_prop = array(0, dim = c(nspp, 2, max_age, nyrs)) # -- ##;; other_food_diet_prop.setZero()#;;           # Other food diet proportion by weight
  #stom_prop_hat = stom_prop_obs#;; stom_prop_hat.setZero()#;;                              # Estimated stomach proportion by weight U (formated following data input)

  # -- 4.10. Suitability components
  avail_food = array(0, dim = c(nspp, 2, max_age, nyrs)) # -- ##;; avail_food.setZero()#;;                                           # Available food to predator
  stom_div_bio = array(0, dim = c(nspp * 2, nspp * 2, max_age, max_age, nyrs))#;; stom_div_bio.setZero()#;;                   # Stomach proportion over biomass#;; U/ (W * N)
  suit_main = array(0, dim = c(nspp * 2, nspp * 2, max_age, max_age, nyrs))#;; suit_main.setZero()#;;                         # Suitability/gamma selectivity of predator age u on prey age a
  suit_other = array(0, dim = c(nspp, 2, max_age, nyrs)) # -- ##;; suit_other.setZero()#;;                                           # Suitability not accounted for by the included prey
  suma_suit = array(0, dim = c(nspp, 2, max_age, nyrs)) # -- ##;; suma_suit.setZero()#;;                                             # Sum of suitabilities
  ...
  /*
   ...
   sum_phi = rep(0, nspp) # -- ##;; sum_phi.setZero()#;;                          # Sum of predator-prey preference coefficients for multinomial transformation
   vulnerability = matrix(0, nspp, nspp)#;; vulnerability.setZero()#;;        # Predator-prey preference coefficients
   vulnerability_other = rep(0, nspp) # -- ##;; vulnerability_other.setZero()#;;  # Preference for other food
   */

  # -- 4.12. Predation components
  M2_at_age = array(0, dim = c(nspp, 2, max_age, nyrs)) # -- ##;; M2_at_age.setZero()#;;                        # Predation mortality at age
  # M2_prop = array(0, dim = c(nspp * 2, nspp * 2, max_age, max_age, nyrs))#;; M2_prop.setZero()#;;     # Relative predation mortality at age from each species at age
  B_eaten_as_prey = array(0, dim = c(nspp, 2, max_age, nyrs)) # -- ##;; B_eaten_as_prey.setZero()#;;            # Biomass eaten as prey via predation
  # B_eaten_as_pred = array(0, dim = c(nspp, 2, max_age, nyrs)) # -- ##;; B_eaten_as_pred.setZero()#;;         # Biomass eaten as predator via predation
  # B_eaten = array(0, dim = c(nspp * 2, nspp * 2, max_age, max_age, nyrs))#;; B_eaten.setZero()#;;     # Biomass of prey eaten via predation by a predator at age
  # N_eaten = array(0, dim = c(nspp * 2, nspp * 2, max_age, max_age, nyrs))#;; N_eaten.setZero()#;;     # Number of prey of age a eaten by predator age u

  # -- 4.13. Kinzey Functional response parameters
  /*
   H_1 = matrix(0, nrow = nspp, ncol = nspp + 1)#;; H_1 = exp(logH_1.array())#;;
   H_1a = rep(0, nspp) # -- ##;; H_1a = exp(logH_1a)#;;
   H_1b = rep(0, nspp) # -- ##;; H_1b = exp(logH_1b)#;;
   H_2 = matrix(0, nspp, nspp)#;; H_2 = exp(logH_2.array())#;;
   H_3 = matrix(0, nspp, nspp)#;; H_3 = exp(logH_3.array())#;;

   array<Type>  N_pred_yrs = array(0, dim = c(nspp, 2, max_age, nyrs)) # -- ##;; N_pred_yrs.setZero()#;;                # Effective numbers of predators for each age of prey FIXME: should be avgN_at_age?
   array<Type>  N_prey_yrs = array(0, dim = c(nspp, 2, max_age, nyrs)) # -- ##;; N_prey_yrs.setZero()#;;                # Effective numbers of prey for each age of predator
   array<Type>  N_pred_eq = array(0, dim = c(nspp, 2, max_age))#;; N_pred_eq.setZero()#;;                        # Effective numbers of predators for each age of prey (styr_pred)
   array<Type>  N_prey_eq = array(0, dim = c(nspp, 2, max_age))#;; N_prey_eq.setZero()#;;                        # Effective numbers of prey for each age of predator
   ...
   array<Type>  Pred_r = array(0, dim = c(nspp, 2, max_age, nyrs)) # -- ##;; Pred_r.setZero()#;;                          # save Pred_ratio values
   array<Type>  Prey_r = array(0, dim = c(nspp, 2, max_age, nyrs)) # -- ##;; Prey_r.setZero()#;;                          # save Prey_ratio values

   array<Type> ration_hat = array(0, dim = c(nspp, 2, max_age, nyrs)) # -- ##;; ration_hat.setZero()#;;                   # Annual ration by predator age each year
   array<Type> ration_hat_ave = array(0, dim = c(nspp, 2, max_age))#;; ration_hat_ave.setZero()#;;                 # Annual ration by predator age averaged over years
   */
  ...
  flt_q = matrix(0, nrow = n_flt, ncol = nyrs_hind)#;; flt_q.setZero()#;;                        # Vector to save q on natural scale
  vector<int> flt_sel_ind = rep(0, n_flt)#;; flt_sel_ind.setZero()#;;                        # Vector to store survey index
  vector<int> flt_type = rep(0, n_flt)#;; flt_type.setZero()#;;                              # Index wether the data are included in the likelihood or not (0 = no, 1 = yes)
  vector<int> flt_sel_type = rep(0, n_flt)#;; flt_sel_type.setZero()#;;                      # Vector to save survey selectivity type
  vector<int> flt_nselages = rep(0, n_flt)#;; flt_nselages.setZero()#;;                      # Vector to save number of ages to estimate non-parametric selectivity (1 = age, 2 = length)
  vector<int> flt_varying_sel = rep(0, n_flt)#;; flt_varying_sel.setZero()#;;                # Vector storing information on wether time-varying selectivity is estimated (0 = no, 1 = random walk with fixed variance, 2 = random effect)
  vector<int> flt_spp = rep(0, n_flt)#;; flt_spp.setZero()#;;                                # Vector to save survey species
  vector<int> flt_sel_age = rep(0, n_flt)#;; flt_sel_age.setZero()#;;                        # Vector to save age first selected (selectivity below this age = 0)
  vector<int> flt_sel_maxage = rep(0, n_flt)#;; flt_sel_maxage.setZero()#;;                  # Vector to save age of max selectivity for normalization (if NA not used)
  vector<int> comp_ll_type = rep(0, n_flt)#;; comp_ll_type.setZero()#;;                      # Vector to save composition type
  vector<int> flt_units = rep(0, n_flt)#;; flt_units.setZero()#;;                            # Vector to save survey units (1 = weight, 2 = numbers)
  vector<int> flt_wt_index = rep(0, n_flt)#;; flt_wt_index.setZero()#;;                      # Vector to save 1st dim of wt to use for weight-at-age
  vector<int> flt_age_transition_index = rep(0, n_flt)#;; flt_age_transition_index.setZero()#;; # Vector to save 3rd dim of age_trans_matrix to use for ALK
  vector<int> flt_q_ind = rep(0, n_flt)#;; flt_q_ind.setZero()#;;                            # Vector storing index of survey q for mapping
  vector<int> est_index_q = rep(0, n_flt)#;; est_index_q.setZero()#;;                            # Vector to save wether or not analytical q is used
  vector<int> index_varying_q = rep(0, n_flt)#;; index_varying_q.setZero()#;;                    # Vector storing information on wether time-varying q is estimated (0 = no, 1 = random walk with fixed variance, 2 = random effect)
  vector<int> est_sigma_index = rep(0, n_flt)#;; est_sigma_index.setZero()#;;                    # Vector to save wether sigma survey is estimated
  vector<int> est_sigma_fsh = rep(0, n_flt)#;; est_sigma_fsh.setZero()#;;                    # Vector to save wether sigma fishery is estimated
  ...
  for(flt = 0#;; flt < n_flt#;; flt++){
    for(yr = 0#;; yr < nyrs_hind#;; yr++){
      ...
      if(est_index_q(flt) == 5){
        ...
        index_q(flt, yr) = exp(index_ln_q(flt) + sum(index_q_mult))#;;
      }
  ...
  for (flt = 0#;; flt < n_flt#;; flt++) {
    ...
    if (sel_type > 0) {
      ...
      if (flt_sel_maxage(flt) >= 0) {
        for (yr = 0#;; yr < nyrs_hind#;; yr++) {
          for(sex = 0#;; sex < nsex(sp)#;; sex++){
            max_sel = sel(flt, sex, flt_sel_maxage(flt), yr)#;; # Get max sel by sex/year (split or otherwise, divides by 1 for ages > maxselage)
  ...
  for (int iter = 0#;; iter < niter#;; iter++) {
    ...
    for ( sp = 0#;; sp < nspp #;; sp++) {
      ...
      case 1: # Random about mean with environmental linkage
        ...
        R_init(sp) = R0(sp) = exp(rec_pars(sp, 0) + sum(srr_mult))#;;
        break#;;
    ...
    /*
     ...
     sexr_denom = 0#;; # Denominator for sex ration across ages
     sexr_nom = 0#;; # Nominator for sex ratio across ages
     for (sp = 0#;; sp < nspp#;; sp++) {
     for (yr = 0#;; yr < nyrs#;; yr++) {
     # Re-initialize
     sexr_denom = 0#;;
    ...
    if (msmMode > 0) {
      ...
      /*
       ...
       log_size_ratio = 0#;;       # Log(mean(predLen@age)/mean(preyLen@age))
    ...
    case 4: # Weight-based lognormal suitability
      ...
      /*
       ...
       Pred_ratio = 0.0#;;          # Predator ratio
       Prey_ratio = 0.0#;;          # Prey ratio
       NS_Z = 0.0#;;                # N(k,yr,a) * survival/Z = 0.0#;;
       Tmort = 0.0#;;               # Mortality on other
       Q_ksum_l = 0.0#;;            # Diet sum
       Term = 0.0#;;                # Linear adjustment for predation
 ...
 */
  ...
  jnll_comp = matrix(0, 19, n_flt)#;; jnll_comp.setZero()#;;  # matrix of negative log-likelihood components
  ...
  for (comp_ind = 0#;; comp_ind < comp_obs.rows()#;; comp_ind++) {
    ...
    # Select sections
    vector<Type> comp_obs_tmp = comp_obs[comp_ind).segment(0, n_comp)#;; # Observed proportion
    vector<Type> comp_hat_tmp = comp_hat[comp_ind).segment(0, n_comp)#;; # Expected proportion
  ...
  for (sp = 0#;; sp < nspp#;; sp++) {
    ...
    if((srr_est_mode == 3) & (srr_pred_fun == 2 | srr_pred_fun == 3)){
      ...
      beta_alpha = ((1 - srr_prior(sp))/ square(srr_prior_sd(sp)) - 1/srr_prior(sp)) * square(srr_prior(sp))#;;
  ...
  for (sp = 0#;; sp < nspp#;; sp++) {
    ...
    for (sp = 0#;; sp < nspp#;; sp++) {
      penalty = 0.0#;;
      nothing_useful =  posfun( (ssb_depletion(sp, nyrs-1) - Plimit(sp)), Type(0.0001), penalty)#;;
