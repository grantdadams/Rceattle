...
//          Climate enhanced Age-based model
//      with Temperature specific Trophic linkages and Energetics
//              Biomass Linkages To The Environment
// AUTHORS:   Kirstin Holsman,Grant Adams, Jim Ianelli
//    This TMB version modified by K. Holsman from Rceattle to include EBS dynamics
//    Rceattle TMB version Modified from CEATTLE ADMB model (Adams et al. 2024  )
//    CEATTL EADMB original version for the EBS (Holsman et al. 2016,2023)
// CITATIONS:
...
// 2. (2023) Holsman, KK, J Ianelli, K Aydin, 2023. Multi-species Stock Assessment for walleye pollock, Pacific cod, and arrowtooth flounder in the Eastern Bering Sea. In Stock Assessment and Fishery Evaluation Report for the Groundfish Resources for the Bering Sea/Aleutian Islands Regions. North Pacific Fishery Management Council, Anchorage, AK
// 3. Adams et al. 2024 (Dissertation and publications)
// ------------------------------------------------------------------------- //
//
// # Holsman & Sulc Changes from Adams et al. 2024
// # 1) adding in predator and prey overlap to the predation submodels
// # 2) adding in env. driven weight at age (Holsman assessment and Holsman et al. 2024)
// # 3) adding in futR receruitment options that include pre- or post-spawning covariate effects
// # 4) addin in HCRs and control rules including the assessment SQ MSM hybrid climate naive target + climate informed
// #   and climate informed status HCR (sensu Holsman et al. 2020 and the MSM assessment)
//
// Changes from 2017 assessment
// 1. Removed option for code to normalize age-transition matrix; Data now needs to be normailized prior to use
...
// 9. Allowed hindcast outside of retrospective estimation years
...
// 14. Added in multiple weights for surveys
// 15. Added in spawning month mortality adjustment
// 16. Removed constant 0.0001 added to M1
// 17. Had the model estimate UobsWtAge
// 18. Added flexibility to fixed n-at-age to have age, sex specific scaling paramters and estimate selectivity

// Modifications from Rceattle V01 (adams et al. 202X)
// ----------------------------------------------------
...
// Added in spawning stock biomass weight as in ADMB CEATTLE 2017 assessment
// Added analytical q for time-varying survey sigma inputs
// All random effect selectivity and catchability deviates are commented out
// Estimate M1 for sex/species as in ADMB CEATTLE 2017 assessment
// Fixed suitability estimation (use hindcast only) as in ADMB CEATTLE 2017 assessment
// Added in non-parametric time varying selectivity similar to Hake
// Added in dynamic reference points as in ADMB CEATTLE 2017 assessment
// Added depletion and F as in ADMB CEATTLE 2017 assessment
//
//  INDEX:
//  0. Load dependencies
...
Type objective_function<Type>::operator() () {
  ...
  //    1 = N*exp(-Z / 2))
  //    2 = N
  DATA_IVECTOR (Wage_mode)                 // used to use data, vonBEE, or est VonBEE
  //    0 = empirical weight at age: set wt_est to wt; for proj wt_est = last wt from the data
  ...
  DATA_IVECTOR( pop_wt_index );           // Dim 3 of wt_est to use for population dynamics
  DATA_IVECTOR( ssb_wt_index );           // Dim 3 of wt_est to use for spawning stock biomass calculation
  DATA_IVECTOR( pop_age_transition_index );// Dim 3 of wt_est to use for age_transition_matrix
  ...
  int sp, sex, age, ln, ksp, k_sex, k_age, k_ln, yr, rsp, r_sex, r_age, r_ln, cov, mf; // k
  ...
  // -- 2.3.5. Weight-at-age
  DATA_ARRAY( wt );                   // Input weight-at-age by year; n = [nweight, sex, nages, nyrs]: FIXME: Change nyrs to nyrs_wt_at_age if data don't span entire bit
  ...
  // -- 2.4.2. Kirstin's von Bertalannfy growth function (VBGF): This is used to calculate future weight-at-age: NOT YET IMPLEMENTED
  // see below
  ...
  // -- 2.4.5  Kirstin's Additions
  DATA_ARRAY (overlap);           // Array of predation overlap (pred,prey, yr)
  DATA_IVECTOR (mf_type);         // does the vonb use 1 sex or 2 sex options?
  DATA_MATRIX (t0);
  DATA_MATRIX (log_mean_d);
  DATA_MATRIX (logK);
  DATA_MATRIX (logH);
  DATA_ARRAY (vonb_cov);             // (sp,sex, ncovars) starting values for VonB env covars
  //DATA_IMATRIX ( vonb_ncov );        // (sp,sex) number of vonB covariates for each (sp,sex)
  DATA_INTEGER ( vonb_ncov );        // number of vonB covariates
  DATA_IARRAY ( vonb_ncov_eval );    // (sp,sex,ncovars) should the covariates be estimated? (allows for no-covariate model)
  DATA_IARRAY ( vonBEE_env_index );  // (sp,sex, ncovars) Indices to link VonBEE to env covars
  ...
  // 3.10. VonB
  //TODO Holsmnan's est VonB
  //PARAMETER_MATRIX (t0_est);
  //PARAMETER_MATRIX (log_mean_d_est);
  //PARAMETER_MATRIX (logK_est);
  //PARAMETER_MATRIX (logH_est);
  //PARAMETER_MATRIX (sigma_wt);
  //PARAMETER_ARRAY (vonb_cov_est);             // (sp,sex, ncovars) starting values for VonB env covars
  ...
  array<Type>   C_MaxAge( nspp, 2, max_age, nyrs ); C_MaxAge.setZero();             // Pre-allocated individual Max consumption in grams per predator-age
  array<Type>   ConsumAge( nspp, 2, max_age, nyrs ); ConsumAge.setZero();           // Pre-allocated individual consumption in grams per predator-age
  ...
  // Kirstin additions
  array<Type>  wt_est(nspp, 2, max_age, nyrs); wt_est.setZero();          // dynamic Weight-at-age by year; n = [nweight, sex, nages, nyrs]: FIXME: Change nyrs to nyrs_wt_at_age if data don't span entire bit
  matrix<Type> sigma(nspp,2); sigma.setZero();                   // Sigma for weight at age fitting
    ...
    for (age = 0; age < nages(sp); age++) {
      for(sex = 0; sex < nsex(sp); sex ++){
        for (yr = 0; yr < nyrs; yr++) {
          // Hindcast
          if(yr < nyrs_hind){
            LbyAge( sp, sex, age, yr) = ( pow( ( 1 / aLW(sp, 0) ), (1 / aLW(sp, 1) ) ) )  * pow( ( wt_est(pop_wt_index(sp), sex, age, yr) * 1000), (1 / aLW(sp, 1))); // W = a L ^ b is the same as (W/a)^(1/b)
          }
          if(yr >= nyrs_hind){
            LbyAge( sp, sex, age, yr) = ( pow( ( 1 / aLW(sp, 0) ), (1 / aLW(sp, 1) ) ) )  * pow( ( wt_est( pop_wt_index(sp), sex, age, (nyrs_hind - 1) ) * 1000), (1 / aLW(sp, 1))); // W = a L ^ b is the same as (W/a)^(1/b)
          }
        }
      }
    }
  ...
  // Subtract 1 from index to deal with indexing start at 0
  Cindex  -= 1;
  vonBEE_env_index -=1;


  // 5.10 calculate weight at age (from Holsman ADMB code)
  if( Wage_mode(sp)!=0 ){
    if( Wage_mode(sp)!=1 ){
      if( Wage_mode(sp)!=2 ){
  ...
  vector<int> flt_wt_index(n_flt); flt_wt_index.setZero();                      // Vector to save 1st dim of wt_est to use for weight-at-age
  ...
  for (flt_ind = 0; flt_ind < n_flt; flt_ind++){
    ...
    flt_accum_age_lower(flt) = fleet_control(flt_ind, 9) - minage(flt_spp(flt));       // Dim1 of wt_est
    ...
    flt_wt_index(flt) = fleet_control(flt_ind, 12) - 1;      // Dim1 of wt_est
  ...
  for (int iter = 0; iter < niter; iter++) {
    ...
    for (sp = 0; sp < nspp; sp++) {
      ...
      if(nsex(sp)  == 1){
        R_sexr(sp) = 1; //FIXME: move to initial calcs section
      }
        ...
        for (age = 0; age < nages(sp); age++){
          ...
          case 0: // Estimated abundance
            ...
            if(age == 0){
             ...
             if(sex ==0){
              NByage(sp, 0, age, 0) = exp(ln_mean_rec(sp) - mort_sum + init_dev(sp, age - 1)) * R_sexr(sp); // NOTE: could be better by having 1 loop and multiplying by exp(init_dev) at the end
             }
             if(sex == 1){
      ...
      NByage(sp, 0, 1, 0) = exp(ln_mean_rec(sp) +  rec_dev(sp, 0) - M1(sp,0,0));
    ...
    for (sp = 0; sp < nspp; sp++) {
      for (age = 0; age < nages(sp); age++) {
        ...
        for (yr = 0; yr < nyrs_hind; yr++) {
          for(sex = 0; sex < nsex(sp); sex ++){
            biomassByage(sp, sex, age, yr) = NByage(sp, sex, age, yr) * wt_est( pop_wt_index(sp), sex, age, yr ); // 6.5.
            biomassSSBByage(sp, age, yr) = NByage(sp, 0, age, yr) * pow(S(sp, 0, age, yr), spawn_month(sp)/12) * wt_est(ssb_wt_index(sp), 0, age, yr ) * pmature(sp, age); // 6.6.
    ...
    for (sp = 0; sp < nspp; sp++) {
      ...
      // Calulcate SPR
      for (age = 0; age < nages(sp); age++) {
        SPR0(sp) +=  NbyageSPR(0, sp, age) *  wt_est( ssb_wt_index(sp), 0, age, (nyrs_hind - 1) ) * pmature(sp, age) * exp(-M(sp, 0, age, nyrs_hind - 1) * spawn_month(sp)/12);
        SPRlimit(sp) +=  NbyageSPR(1, sp, age) *  wt_est( ssb_wt_index(sp), 0, age, (nyrs_hind - 1) ) * pmature(sp, age) * exp(-(M(sp, 0,  age, nyrs_hind - 1) + FlimitSPR(sp, 0,  age)) * spawn_month(sp)/12);
        SPRtarget(sp) +=  NbyageSPR(2, sp, age) *  wt_est( ssb_wt_index(sp), 0, age, (nyrs_hind - 1) ) * pmature(sp, age) * exp(-(M(sp, 0,  age, nyrs_hind - 1) + FtargetSPR(sp, 0,  age)) * spawn_month(sp)/12);
      }
    }
    ...
    for (sp = 0; sp < nspp; sp++) {
      ...
      for (yr = 1; yr < nyrs; yr++) { // No initial
        ...
        for (age = 0; age < nages(sp); age++) {
          DynamicSB0(sp, yr) +=  DynamicNByage0(sp, 0, age, yr) *  wt_est( ssb_wt_index(sp), 0, age, yr_ind ) * pmature(sp, age) * exp(-M(sp, 0, age, yr) * spawn_month(sp)/12);

          for(sex = 0; sex < nsex(sp); sex ++){
            DynamicB0(sp, yr) +=  DynamicNByage0(sp, sex,age, yr) *  wt_est( ssb_wt_index(sp), sex, age, yr_ind );
          }

          // Dynamic SB with F (loop across fleets)
          for (flt = 0; flt < n_flt; flt++) {
            if(flt_spp(flt) == sp){
              DynamicSBF(sp, yr) +=  DynamicNByageF(yr, sp, age, yr) *  wt_est( ssb_wt_index(sp), 0, age, yr_ind ) * pmature(sp, age) * exp(-(M(sp, 0, age, yr) + sel(flt, 0, age, yr_ind) * proj_F_prop(flt) * Ftarget(sp, yr)) * spawn_month(sp)/12); // Fixme - add F
            }
          }
        }
      }
      ...
      // Calculate SB0
      for (age = 0; age < nages(sp); age++) {
        SB0(sp) +=  NByage0(sp, 0, age, nyrs-1) *  wt_est( ssb_wt_index(sp), 0, age, nyrs_hind - 1 ) * pmature(sp, age) * exp(-M(sp, 0, age, nyrs-1) * spawn_month(sp)/12);
        for(sex = 0; sex < nsex(sp); sex ++){
          B0(sp) +=  NByage0(sp, sex, age, nyrs-1) *  wt_est( pop_wt_index(sp), sex, age, nyrs_hind - 1 );
        }
      }
      ...
      for (yr = nyrs_hind; yr < nyrs; yr++){
        ...
        for (age = 0; age < nages(sp); age++) {
          for(sex = 0; sex < nsex(sp); sex ++){
            ...
            biomassByage(sp, sex, age, yr) = NByage(sp, sex, age, yr) * wt_est( pop_wt_index(sp), sex, age, nyrs_hind-1 ); // 6.5.
            biomass(sp, yr) += biomassByage(sp, sex, age, yr);
          }

          // -- 6.3.3. FORECAST SSB
          biomassSSBByage(sp, age, yr) = NByage(sp, 0, age, yr) * pow(S(sp, 0, age, yr), spawn_month(sp)/12) * wt_est(ssb_wt_index(sp), 0, age, nyrs_hind-1 ) * pmature(sp, age); // 6.6.
          biomassSSB(sp, yr) += biomassSSBByage(sp, age, yr);
        }
      }
    }
    ...
    // 7. RATION EQUATIONS                                                       //
    // ------------------------------------------------------------------------- //
    // Based on Holsman et al. 2016,2020 / EBS MSM stock assessment
    ...
    // 7.3. Calculate historical ration
    // Based on Holsman et al. 2016 approach for bioenergetics based M2
    for (sp = 0; sp < nspp; sp++) {
      for(sex = 0; sex < nsex(sp); sex++) {
        for (age = 0; age < nages(sp); age++) {
          for (yr = 0; yr < nyrs; yr++) {
            ...
            // fday = number of foraging days per year

            // KKHFIX to match Holsman CEATTLE model where projections are wt_est at age from dynamic/calculated wt_est
            C_MaxAge(sp, sex, age, yr) = CA(sp) * pow(wt_est( pop_wt_index(sp), sex, age, yr ) * Type(1000), 1 + CB( sp )) * fT(sp, yr) * fday( sp );
            //   C_max = CA * W ^ 1+CB; and C_max * f(T) * wt_est * fday g/pred/yr ; where C_max is grams consumed per grams of predator per day
    ...
    if (msmMode > 0) {
      ...
      if (suitMode == 0) {
                  ...
                  for (r_age = 0; r_age < nages(rsp); r_age++) {    // Predator age loop
                    for (k_age = 0; k_age < nages(ksp); k_age++) {  // Prey age loop
                      ...
                      if(AvgN(ksp, k_sex, k_age, yr) > 0){
                        ...
                        // U =
                        // if(msmMode == 2){
                        // // KKH FIX - why divide again by AvgN?? Removing this
                      ...
                      // KKHFIX this should be a non-issue
                      if(!isFinite(suit_tmp(rsp + (nspp * r_sex), ksp + (nspp * k_sex), r_age, k_age, yr))){
                        suit_tmp(rsp + (nspp * r_sex), ksp + (nspp * k_sex), r_age, k_age, yr) = 0;
                      }

                      if (wt_est(pop_wt_index(ksp), k_sex, k_age, yr_ind ) != 0) {
                        stom_div_bio2(rsp + (nspp * r_sex), ksp + (nspp * k_sex), r_age, k_age, yr) = suit_tmp(rsp + (nspp * r_sex), ksp + (nspp * k_sex), r_age, k_age, yr) / wt_est( pop_wt_index(ksp), k_sex, k_age, yr_ind );
                      }else{
                        stom_div_bio2(rsp + (nspp * r_sex), ksp + (nspp * k_sex), r_age, k_age, yr) = 0;
                      }
        ...
        // MSVP plus pred/prey overlap sensu Holsman et al.
        // KKHFIX Fixed to include PredPrey overlap and dynamic (future) wt_est at age
        suit_main.setZero();
        suit_other.setZero();
        for (rsp = 0; rsp < nspp; rsp++) {                          // Predator species loop
              ...
              for (ksp = 0; ksp < nspp; ksp++) {                    // Prey species loop
                for(k_sex = 0; k_sex < nsex(ksp); k_sex++){         // Prey sex loop
                  for (k_age = 0; k_age < nages(ksp); k_age++) {    // Prey age loop
                    ...
                    // Remove NAs from crashing populations ?? KKH: this shouldn't have NAS
                    ...
                    // Other suitability
                    for (yr = 0; yr < nyrs; yr++) {
                      suit_other(rsp, r_sex, r_age, yr) += overlap(rsp, ksp , yr)* suit_main(rsp + (nspp * r_sex), ksp + (nspp * k_sex), r_age, k_age, yr); // KKHFIX - include overlap indices
                    }
                  }
                }
              }
                    ...
                    for (k_age = 0; k_age < nages(ksp); k_age++) {     // Prey age
                      for (yr = 0; yr < nyrs; yr++) {                  // Year loop
                        ...
                        case 2: // Weight-based GAMMA suitability
                          log_size_ratio = log(wt_est(pop_wt_index(rsp), r_sex, r_age, yr_ind) / wt_est(pop_wt_index(ksp), k_sex, k_age, yr_ind)); // Log ratio of weights
                          break;
                        }
                        if(log_size_ratio > 0){
                ...
                for (ksp = 0; ksp < nspp; ksp++) {                  // Prey species loop
                  for(k_sex = 0; k_sex < nsex(ksp); k_sex++){
                    for (k_age = 0; k_age < nages(ksp); k_age++) {    // Prey age loop
                      ...
                      // original ADMB code: avail_food(pred,i,pred_age) += suit_main(pred,pred_age,prey,prey_age)*overlap(i,pred,prey) *AvgN(prey,i,prey_age) * mn_wt_stom(pred,pred_age,prey,prey_age);

                      //Rceattle version: avail_food(rsp, r_sex, r_age, yr) += suit_main(rsp + (nspp * r_sex), ksp + (nspp * k_sex), r_age, k_age, yr) * pow(AvgN(ksp, k_sex, k_age, yr), msmMode) * wt_est(pop_wt_index(ksp), k_sex, k_age, yr_ind) ; // FIXME - include overlap indices: FIXME - mn_wt_stom?NO
                      avail_food(rsp, r_sex, r_age, yr) += suit_main(rsp + (nspp * r_sex), ksp + (nspp * k_sex), r_age, k_age, yr) * overlap(rsp,ksp,yr)*pow(AvgN(ksp, k_sex, k_age, yr), msmMode) * wt_est(pop_wt_index(ksp), k_sex, k_age, yr) ; // KKHFIXed - include overlap indices
                  ...
                  for (r_age = 0; r_age < nages(rsp); r_age++) {    // Predator age loop
                    for (yr = 0; yr < nyrs; yr++) {                       // Year loop
                      ...
                      if(avail_food(rsp, r_sex, r_age, yr) > 0){
                        ...
                        switch(msmMode){
                        case 1: // Type 2 MSVPA plus Holsman et al. approach for pred/prey overlap
                          //KKHFIX added overlap
                          //CEATTLE CODE Mtmp += (AvgN(pred,i,pred_age)*overlap(i,pred,prey) * ration2Age(pred,i,pred_age)*suit_main(pred,pred_age,prey,prey_age))/avail_food(pred,i,pred_age);
                          M2(ksp, k_sex, k_age, yr) += (AvgN(rsp, r_sex, r_age, yr) * overlap(rsp,ksp,yr) * ration(rsp, r_sex, r_age, yr) * suit_main(rsp + (nspp * r_sex), ksp + (nspp * k_sex), r_age, k_age, yr)) / avail_food(rsp, r_sex, r_age, yr); // #FIXME - include indices of overlap
                          M2_prop(rsp + (nspp * r_sex), ksp + (nspp * k_sex), r_age, k_age, yr) = (AvgN(rsp, r_sex, r_age, yr) * overlap(rsp,ksp,yr) * ration(rsp, r_sex, r_age, yr) * suit_main(rsp + (nspp * r_sex), ksp + (nspp * k_sex), r_age, k_age, yr)) / avail_food(rsp, r_sex, r_age, yr);

                          // KKHFIX biomass eaten should not be divided by avail food?? Kir check this, seems rather it is the ration * pred_biomass * pref for a given prey
                          B_eaten_as_prey(ksp, k_sex, k_age, yr) += AvgN(ksp, k_sex, k_age, yr) * wt_est(pop_wt_index(ksp), k_sex, k_age, yr_ind) * AvgN(rsp, r_sex, r_age, yr) * overlap(rsp,ksp,yr) * ration(rsp, r_sex, r_age, yr) * suit_main(rsp + (nspp * r_sex), ksp + (nspp * k_sex), r_age, k_age, yr) / avail_food(rsp, r_sex, r_age, yr);
                          B_eaten(rsp + (nspp * r_sex), ksp + (nspp * k_sex), r_age, k_age, yr) = AvgN(ksp, k_sex, k_age, yr) * wt_est(pop_wt_index(ksp), k_sex, k_age, yr_ind) * AvgN(rsp, r_sex, r_age, yr) * overlap(rsp,ksp,yr)* ration(rsp, r_sex, r_age, yr) * suit_main(rsp + (nspp * r_sex), ksp + (nspp * k_sex), r_age, k_age, yr) / avail_food(rsp, r_sex, r_age, yr);

                          diet_prop_weight_hat(rsp + (nspp * r_sex), ksp + (nspp * k_sex), r_age, k_age, yr) = AvgN(ksp, k_sex, k_age, yr) * wt_est(pop_wt_index(ksp), k_sex, k_age, yr_ind) * suit_main(rsp + (nspp * r_sex), ksp + (nspp * k_sex), r_age, k_age, yr)/ avail_food(rsp, r_sex, r_age, yr);
                          break;


                        case 2: // Type 3 MSVPA
                          // KKHFIX added overlap
                          M2(ksp, k_sex, k_age, yr) += pow(AvgN(ksp, k_sex, k_age, yr) , msmMode) * wt_est(pop_wt_index(ksp), k_sex, k_age, yr_ind) * (AvgN(rsp, r_sex, r_age, yr) * ration(rsp, r_sex, r_age, yr) * overlap(rsp,ksp,yr)
                                                                                                                                                     * suit_main(rsp + (nspp * r_sex), ksp + (nspp * k_sex), r_age, k_age, yr)) / avail_food(rsp, r_sex, r_age, yr) / (AvgN(ksp, k_sex, k_age, yr) * wt_est(pop_wt_index(ksp), k_sex, k_age, yr_ind)); // #FIXME - include indices of overlap
                          M2_prop(rsp + (nspp * r_sex), ksp + (nspp * k_sex), r_age, k_age, yr) = pow(AvgN(ksp, k_sex, k_age, yr), msmMode) * wt_est(pop_wt_index(ksp), k_sex, k_age, yr_ind) * (AvgN(rsp, r_sex, r_age, yr) * ration(rsp, r_sex, r_age, yr)  * overlap(rsp,ksp,yr) * suit_main(rsp + (nspp * r_sex), ksp + (nspp * k_sex), r_age, k_age, yr)) / avail_food(rsp, r_sex, r_age, yr) / (AvgN(ksp, k_sex, k_age, yr) * wt_est(pop_wt_index(ksp), k_sex, k_age, yr_ind));
                          B_eaten_as_prey(ksp, k_sex, k_age, yr) += pow(AvgN(ksp, k_sex, k_age, yr), msmMode) * wt_est(pop_wt_index(ksp), k_sex, k_age, yr_ind) * AvgN(rsp, r_sex, r_age, yr) * ration(rsp, r_sex, r_age, yr) * overlap(rsp,ksp,yr)  * suit_main(rsp + (nspp * r_sex), ksp + (nspp * k_sex), r_age, k_age, yr) / avail_food(rsp, r_sex, r_age, yr);
                          B_eaten(rsp + (nspp * r_sex), ksp + (nspp * k_sex), r_age, k_age, yr) = pow(AvgN(ksp, k_sex, k_age, yr), msmMode) * wt_est(pop_wt_index(ksp), k_sex, k_age, yr_ind) *  AvgN(rsp, r_sex, r_age, yr) * ration(rsp, r_sex, r_age, yr) * overlap(rsp,ksp,yr)  * suit_main(rsp + (nspp * r_sex), ksp + (nspp * k_sex), r_age, k_age, yr) / avail_food(rsp, r_sex, r_age, yr);
                          diet_prop_weight_hat(rsp + (nspp * r_sex), ksp + (nspp * k_sex), r_age, k_age, yr) = pow(AvgN(ksp, k_sex, k_age, yr) , msmMode) * wt_est(pop_wt_index(ksp), k_sex, k_age, yr_ind) * suit_main(rsp + (nspp * r_sex), ksp + (nspp * k_sex), r_age, k_age, yr) / avail_food(rsp, r_sex, r_age, yr) / (AvgN(ksp, k_sex, k_age, yr) * wt_est(pop_wt_index(ksp), k_sex, k_age, yr_ind));
                          break;
                        }
                      }
                    }
                  ...
                  if (ksp < nspp) {
                    for(k_sex = 0; k_sex < nsex(ksp); k_sex++){
                      for (k_age = 0; k_age < nages(ksp); k_age++) {
                        ...
                        B_eaten(rsp + (nspp * r_sex), ksp + (nspp * k_sex), r_age, k_age, yr) = M2_prop(rsp + (nspp * r_sex), ksp + (nspp * k_sex), r_age, k_age, yr) * AvgN(ksp, k_sex, k_age, yr) * wt_est(pop_wt_index(ksp), k_sex, k_age, yr_ind);
    ...
    for(srv_ind = 0; srv_ind < srv_biom_ctl.rows(); srv_ind++){
      ...
      for (age = 0; age < nages(sp); age++) {
        for(sex = 0; sex < nsex(sp); sex++){
          // Weight
          if(flt_units(srv) == 1){
            srv_bio_hat(srv_ind) += NByage(sp, sex, age, flt_yr) * exp( - (mo/12) * Zed(sp, sex, age, flt_yr)) * sel(srv, sex, age, yr_ind) * wt_est( flt_wt_index(srv), sex, age, yr_ind );
          }
          // Numbers
          if(flt_units(srv) == 2){
    ...
    for(fsh_ind = 0; fsh_ind < fsh_biom_ctl.rows(); fsh_ind++){
      ...
      for(sex = 0; sex < nsex(sp); sex ++){
        for (age = 0; age < nages(sp); age++) {
          ...
          // By weight
          if(flt_units(flt) == 1){
            fsh_bio_hat(fsh_ind) += F_flt_age(flt, sex, age, flt_yr) / Zed(sp, sex, age, flt_yr) * (1 - exp(-Zed(sp, sex, age, flt_yr))) * NByage(sp, sex, age, flt_yr) * wt_est( flt_wt_index(flt), sex, age, yr_ind ); // 5.5.
          }
  ...
  // Slot 8 -- Survey catchability prior
  // Slot 9 -- Survey catchability annual deviates
  // -- Priors/penalties
  ...
  for(flt = 0; flt < n_flt; flt++){
    ...
    if( est_srv_q(flt) == 2){
      jnll_comp(8, flt) -= dnorm(ln_srv_q(flt) - square(sigma_srv_q(flt))/2, ln_srv_q_prior(flt), sigma_srv_q(flt), true);
    }

    // Penalized/random deviate likelihood
    if(((srv_varying_q(flt) == 1) | (srv_varying_q(flt) == 2)) & (flt_type(flt) == 2)){
      for(yr = 0; yr < nyrs_hind; yr++){
        jnll_comp(9, flt) -= dnorm(ln_srv_q_dev(flt, yr), Type(0.0), time_varying_sigma_srv_q(flt), true );
      }
    }
    ...
    if((srv_varying_q(flt) == 4) & (flt_type(flt) == 2)){
      ...
      jnll_comp(9, flt) += square(q_dev_sum) * 10000;

      for(yr = 1; yr < nyrs_hind; yr++){
        jnll_comp(9, flt) -= dnorm(ln_srv_q_dev(flt, yr) - ln_srv_q_dev(flt, yr-1), Type(0.0), time_varying_sigma_srv_q(flt), true );
      }
    }
  } // End q loop
  ...
  // 12.10. Wt at age components
  REPORT( wt_est );
  REPORT( wt );

  // -- 12.11. Kinzey predation functions
  REPORT( H_1 );
  REPORT( H_1a );
  REPORT( H_1b );
  REPORT( H_2 );
