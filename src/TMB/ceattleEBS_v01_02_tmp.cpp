...
Type objective_function<Type>::operator() () {
  ...
  DATA_INTEGER(rec_mode);

  // !2  default Holsman's approach based on 4 modes of recruitment (mean, liner, ricker, bevholt)
  //            with and with out enviro covariates
  // 0  mean rec +rec_dev
  // 1  mean rec
  // 2  mean linear model
  //
  ...
  //DATA_IVECTOR( estDynamics );            // Index indicating whether the population parameters are estimated (0), numbers-at-age are provided (1), or an index of numbers-at-age multiplied by an estimated scalar is used (2)
  ...
  DATA_IVECTOR (Wage_mode)                // Holsman: use data, vonBEE, or est VonBEE
  ...
  DATA_INTEGER(DynamicHCR);               // TRUE/FALSE. whether to use static or dynamic reference points (default = FALSE)
  ...
  int ncnt;    // Pointers
  //Type tmp_othersuit = 0 ;              // Holsman addition
  ...
  // -- 2.3.5. Weight-at-age
  DATA_ARRAY( wt );                       // Holsman: Input weight-at-age by year; n = [nweight, sex, nages, nyrs]: FIXME: Change nyrs to nyrs_wt_at_age if data don't span entire bit

  // 2.3.6. Diet data
  DATA_VECTOR( fday );                    // number of foraging days for each predator; n = [1, nspp]
  DATA_ARRAY( Pyrs );                     // Relative-foraging rate;  n = [nspp, nyrs+1, nages, yrs]: #FIXME - Assuming this is the same as Pby_yr?
  ...
  DATA_VECTOR( CA );                      // wt_est specific intercept of Cmax=CA*W^CB; n = [1, nspp]
  DATA_VECTOR( CB );                      // wt_est specific slope of Cmax=CA*W^CB; n = [1, nspp]
  ...
  // -- 2.4.2. Holsman's von Bertalannfy growth function with env (VBGF): This is used to calculate future weight-at-age: NOT YET IMPLEMENTED
  // see below
  ...
  DATA_INTEGER ( vonb_ncov );         // (sp,sex) number of vonB covariates
  DATA_IARRAY ( vonb_ncov_eval );     // (sp,sex,vonb_ncov) should the covariates be estimated? (allows for no-covariate model)
  DATA_IARRAY ( vonBEE_env_index );   // (sp,sex, vonb_ncov) Indices to link VonBEE to env covars

  DATA_ARRAY (rec_cov);               // (sp, ncovars) starting values for rec env covars
  DATA_INTEGER ( rec_ncov );          // number of rec covariates
  DATA_IMATRIX ( rec_ncov_eval );     // (sp,rec_ncov) should the covariates be estimated in the recruitment models? (allows for no-covariate model)
  DATA_IMATRIX ( rec_env_index );     // (sp, rec_ncov) Indices to link recruitment to env covars
  ...
  /*
   ...
   // Hindcast
   if(yr < nyrs_hind){
   LbyAge( sp, sex, age, yr) = ( pow( ( 1 / aLW(sp, 0) ), (1 / aLW(sp, 1) ) ) )  * pow( ( wt_est(pop_wt_index(sp), sex, age, yr) * 1000.0), (1 / aLW(sp, 1))); // W = a L ^ b is the same as (W/a)^(1/b)
   }
   if(yr >= nyrs_hind){
   LbyAge( sp, sex, age, yr) = ( pow( ( 1 / aLW(sp, 0) ), (1 / aLW(sp, 1) ) ) )  * pow( ( wt_est( pop_wt_index(sp), sex, age, (nyrs_hind - 1) ) * 1000.0), (1 / aLW(sp, 1))); // W = a L ^ b is the same as (W/a)^(1/b)
   }
   }
   }
   }
  ...
  // THIS NOW done in rearrange dat
  //vonBEE_env_index -=1;
  Rcout << std::endl << "rec_env_index " << rec_env_index << std::endl;
  //rec_env_index -= 1;
  Rcout << std::endl << "rec_env_index is now" << rec_env_index << std::endl;
  // start indexing at 0 not 1
  ...
  vector<int> flt_type(n_flt); flt_type.setZero();                              // Index whether the data are included in the likelihood or not (0 = no, 1 = yes)
  ...
  vector<int> flt_varying_sel(n_flt); flt_varying_sel.setZero();                // Vector storing information on whether time-varying selectivity is estimated (0 = no, 1 = random walk with fixed variance, 2 = random effect)
  ...
  vector<int> est_srv_q(n_flt); est_srv_q.setZero();                            // Vector to save whether or not analytical q is used
  vector<int> srv_varying_q(n_flt); srv_varying_q.setZero();                    // Vector storing information on whether time-varying q is estimated (0 = no, 1 = random walk with fixed variance, 2 = random effect)
  vector<int> est_sigma_srv(n_flt); est_sigma_srv.setZero();                    // Vector to save whether sigma survey is estimated
  vector<int> est_sigma_fsh(n_flt); est_sigma_fsh.setZero();                    // Vector to save whether sigma fishery is estimated
  ...
  for (flt_ind = 0; flt_ind < n_flt; flt_ind++){
    ...
    est_sigma_srv(flt) = fleet_control(flt_ind, 17);         // whether to estimate standard deviation of survey time series
    est_sigma_fsh(flt) = fleet_control(flt_ind, 18);         // whether to estimate standard deviation of fishery time series
    sel_norm_age(flt) =  flt_nselages(flt) - minage(flt_spp(flt));  // Age to normalize logistic selectivities
  }

  // 5.6. Holsman Calculate temperature to use
  ...
  // }
  //
  // // map the env index to the model years (in case there is a mismatch)
  // for(int i = 0; i < env_index.cols(); i++){
  //   yr_ind = 0;
  ...
  for (int iter = 0; iter < niter; iter++) {
    ...
    for (sp = 0; sp < nspp; sp++) {
      ...
      // Calulcate SPR
      for (age = 0; age < nages(sp); age++) {
        SPR0(sp) +=  NbyageSPR(0, sp, age) *  wt_est( ssb_wt_index(sp), 0, age, (nyrs_hind - 1) ) * pmature(sp, age) * exp(-M(sp, 0,  age, nyrs_hind - 1) * spawn_month(sp)/12.0);
        SPRlimit(sp) +=  NbyageSPR(1, sp, age) *  wt_est( ssb_wt_index(sp), 0, age, (nyrs_hind - 1) ) * pmature(sp, age) * exp(-(M(sp, 0,  age, nyrs_hind - 1) + Flimit_age_spp(sp, 0,  age, nyrs_hind - 1)) * spawn_month(sp)/12.0);
        SPRtarget(sp) +=  NbyageSPR(2, sp, age) *  wt_est( ssb_wt_index(sp), 0, age, (nyrs_hind - 1) ) * pmature(sp, age) * exp(-(M(sp, 0,  age, nyrs_hind - 1) + Ftarget_age_spp(sp, 0,  age, nyrs_hind - 1)) * spawn_month(sp)/12.0);
        SPRFinit(sp) +=  NbyageSPR(3, sp, age) *  wt_est( ssb_wt_index(sp), 0, age, 0) * pmature(sp, age) * exp(-(M1(sp, 0,  age) + Finit(sp)) * spawn_month(sp)/12.0);
      }
    }
    ...
    if(rec_mode!=-9){
      ...
      // // default for EBS CEATTLE model, Holsman's approach based on futR
      // // env_index_hat(yr,rec_env_index(sp,cov));
      // srr_mult = 0;
    ...
    for (sp = 0; sp < nspp; sp++) {
      for (age = 0; age < nages(sp); age++){
        ...
        default:
          ...
          /*
           biomassByage(sp, sex, age, 0) = NByage(sp, sex, age, 0) * wt_est( pop_wt_index(sp), sex, age, 0);
           biomass(sp, 0) += biomassByage(sp, sex, age, 0);
           */
          biomass(sp, 0) += NByage(sp, sex, age, 0) * wt_est( pop_wt_index(sp), sex, age, 0);
        }

        // -- 6.5.4. Estimated initial female SSB
        /*
         biomassSSBByage(sp, age, 0) = NByage(sp, 0, age, 0) * pow(S(sp, 0, age, 0), spawn_month(sp)/12.0) * wt_est(ssb_wt_index(sp), 0, age, 0 ) * pmature(sp, age); // 6.6.
         biomassSSB(sp, 0) += biomassSSBByage(sp, age, 0);
         */

        biomassSSB(sp, 0) += NByage(sp, 0, age, 0) * pow( exp(-Zed(sp, 0, age, 0)), spawn_month(sp)/12.0) * wt_est(ssb_wt_index(sp), 0, age, 0 ) * pmature(sp, age); // 6.6.
      }
    }
      ...
      for (yr = 1; yr < nyrs_hind; yr++) {
        ...
        for (age = 0; age < nages(sp); age++){
          ...
          default:
            ...
            /*
             biomassByage(sp, sex, age, yr) = NByage(sp, sex, age, yr) * wt_est( pop_wt_index(sp), sex, age, yr );
             biomass(sp, yr) += biomassByage(sp, sex, age, yr);
             */
            biomass(sp, yr) += NByage(sp, sex, age, yr) * wt_est( pop_wt_index(sp), sex, age, yr );
          }

          // -- 6.6.4. Estimated female SSB
          /*
           biomassSSBByage(sp, age, yr) = NByage(sp, 0, age, yr) * pow(S(sp, 0, age, yr), spawn_month(sp)/12.0) * wt_est(ssb_wt_index(sp), 0, age, yr ) * pmature(sp, age); // 6.6.
           biomassSSB(sp, yr) += biomassSSBByage(sp, age, yr);
           */
          biomassSSB(sp, yr) += NByage(sp, 0, age, yr) * pow(exp(-Zed(sp, 0, age, yr)), spawn_month(sp)/12.0) * wt_est(ssb_wt_index(sp), 0, age, yr ) * pmature(sp, age); // 6.6.
        }
      }
    }
    ...
    for (sp = 0; sp < nspp; sp++) {
      for (yr = 0; yr < nyrs; yr++) {
        ...
        for (age = 0; age < nages(sp); age++) {
          SB0(sp, yr) +=  NByage0(sp, 0, age, yr) *  wt_est( ssb_wt_index(sp), 0, age, nyrs_hind - 1 ) * pmature(sp, age) * exp(-M(sp, 0, age, yr) * spawn_month(sp)/12.0);
          SBF(sp, yr) +=  NByageF(sp, 0, age, yr) *  wt_est( ssb_wt_index(sp), 0, age, nyrs_hind - 1 ) * pmature(sp, age) * exp(-(M(sp, 0, age, yr) + Ftarget_age_spp(sp, 0, age, yr)) * spawn_month(sp)/12.0);
          DynamicSB0(sp, yr) +=  DynamicNByage0(sp, 0, age, yr) *  wt_est( ssb_wt_index(sp), 0, age, yr_ind ) * pmature(sp, age) * exp(-M(sp, 0, age, yr) * spawn_month(sp)/12.0);
          DynamicSBF(sp, yr) +=  DynamicNByageF(sp, 0, age, yr) *  wt_est( ssb_wt_index(sp), 0, age, yr_ind ) * pmature(sp, age) * exp(-(M(sp, 0, age, yr) + Ftarget_age_spp(sp, 0, age, yr)) * spawn_month(sp)/12.0);

          for(sex = 0; sex < nsex(sp); sex ++){
            B0(sp, yr) +=  NByage0(sp, sex, age, yr) *  wt_est( ssb_wt_index(sp), sex, age, nyrs_hind - 1 );
        ...
        for (age = 0; age < nages(sp); age++) {
          ...
          default: // Wrong estDynamics
            ...
            /*
             ...
             biomass(sp, yr) += biomassByage(sp, sex, age, yr);
             */
            biomass(sp, yr) += NByage(sp, sex, age, yr) * wt_est( pop_wt_index(sp), sex, age, nyrs_hind-1 ); // 6.5.
          } // End sex loop

          // -- 6.9.5. FORECAST SSB (SUM ACROSS AGES)
          /*
           biomassSSBByage(sp, age, yr) = NByage(sp, 0, age, yr) * pow(S(sp, 0, age, yr), spawn_month(sp)/12.0) * wt_est(ssb_wt_index(sp), 0, age, nyrs_hind-1 ) * pmature(sp, age); // 6.6.
           biomassSSB(sp, yr) += biomassSSBByage(sp, age, yr);
           */
          biomassSSB(sp, yr) += NByage(sp, 0, age, yr) * pow(exp(-Zed(sp, 0, age, yr)), spawn_month(sp)/12.0) * wt_est(ssb_wt_index(sp), 0, age, nyrs_hind-1 ) * pmature(sp, age); // 6.6.
        }
      }
    }
      ...
      for(sex = 0; sex < nsex(sp); sex++) {
        for (age = 0; age < nages(sp); age++) {
          for (yr = 0; yr < nyrs; yr++) {
            ...
            // calculate theoretical max Consumption
            C_MaxAge(sp, sex, age, yr) = CA(sp) * pow(wt_est( pop_wt_index(sp), sex, age, yr ) * Type(1000.0), 1 + CB( sp )) * fT(sp, yr) * fday( sp );
    ...
    for(srv_ind = 0; srv_ind < srv_biom_ctl.rows(); srv_ind++){
      ...
      for (age = 0; age < nages(sp); age++) {
        for(sex = 0; sex < nsex(sp); sex++){
          // Weight
          if(flt_units(srv) == 1){
            srv_bio_hat(srv_ind) += NByage(sp, sex, age, flt_yr) * exp( - (mo/12.0) * Zed(sp, sex, age, flt_yr)) * sel(srv, sex, age, yr_ind) * wt_est( flt_wt_index(srv), sex, age, yr_ind );
          }
          // Numbers
          if(flt_units(srv) == 2){
    ...
    for(fsh_ind = 0; fsh_ind < fsh_biom_ctl.rows(); fsh_ind++){
      ...
      for(sex = 0; sex < nsex(sp); sex ++){
        for (age = 0; age < nages(sp); age++) {
          ...
          // By weight
          if(flt_units(flt) == 1){
            fsh_bio_hat(fsh_ind) += F_flt_age(flt, sex, age, flt_yr) / Zed(sp, sex, age, flt_yr) * (1.0 - exp(-Zed(sp, sex, age, flt_yr))) * NByage(sp, sex, age, flt_yr) * wt_est( flt_wt_index(flt), sex, age, yr_ind ); // 5.5.
            expl_bio_hat(fsh_ind) += NByage(sp, sex, age, flt_yr) * wt_est( flt_wt_index(flt), sex, age, yr_ind ) * sel(flt, sex, age, yr_ind) * proj_F_prop(flt); // FIXME using last year of selectivity;
          }
    ...
    for (flt = 0; flt < n_flt; flt++) {
      ...
      if(flt_type(flt) == 1){ //Fishery only
        for (yr = 0; yr < nyrs; yr++) {
          ...
          for (age = 0; age < nages(sp); age++) {
            for(sex = 0; sex < nsex(sp); sex ++){
              biomassExpl(sp, yr) += NByage(sp, sex, age, yr) * wt_est( flt_wt_index(flt), sex, age, yr_ind ) * sel(flt, sex, age, yr_ind) * proj_F_prop(flt); // FIXME using last year of selectivity;
            }
          }
        }
      }
