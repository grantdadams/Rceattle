...
`log_K`, `log_Linf`, or `log_L1`, and it can also carry priors on the
linked coefficients.
...
The prior on `(Intercept)` is effectively a prior on the base `log_K`
value, because intercept-bearing linkages replace the base parameter and
carry its level.
