...
**They all report the same way.** Printing any of them opens with one
line — the object, an overall status, and a verdict — before the detail:
    ...
    <Rceattle self-test>  status: NOTE
      47 of 50 simulation(s) converged
              convergence  n
      [OK  ]  OK           41
      [NOTE]  NOTE          6
      observations redrawn, processes held at their fitted values: the spread is a lower bound

    <Rceattle profile>  status: WARN
      sigmaR over 29 grid point(s); all converged
      minimum -log L : 1274.8831  at slot_1 = 1.5
      within 1.92 of the minimum : 1.3 to >=1.5
      the minimum is at a grid edge (slot_1), so the grid does not bracket it -- widen `values`

The severity words are the same four everywhere — `OK`, `NOTE`, `WARN`,
`FAIL` — and the overall status is the worst present. The **return
values are unchanged**: each object is still the data frame or list it
always was, so `retro$mohns`, `jit$nll`, `diag$sdnr`, `prof$grid$slot_1`
and `sims[["Sim_1"]]` index exactly as before, and `c(sims, list(fit))`
still gives a plain list of fits to plot. Only the printing has an
opinion.

Four of these carry a threshold you can set. `print(retro, band = )` is
Mohn’s rho threshold — the default `0.2` is the rule of thumb below, and
Hurtado-Ferro et al. (2015) give asymmetric, life-history-dependent
alternatives (−0.15 to 0.20 long-lived, −0.22 to 0.30 short-lived),
which is why it is an argument. `print(jit, tol = )` is how close an
objective must be to count as having reached the same optimum.
`print(sims, rate = )` is the fraction of simulations that must converge
before a self-test counts as clean. `print(prof, cutoff = )` bounds the
reported interval; the default 1.92 is the 95% profile-likelihood cutoff
for one parameter — half the 0.95 quantile of a chi-square on one degree
of freedom.

Two of them report something the numbers alone cannot. A self-test drops
its non-converged runs before returning, so the length of what comes
back is not the number attempted — the header gives the rate against
`nsim`, and the last line says what generated the replicates, since with
processes held fixed the spread carries observation error only. A
profile whose minimum sits at the first or last grid value has run out
of grid rather than found the optimum; it plots as an ordinary curve, so
the print says so and asks for a wider `values`.
...
- **`hessian_conditioning`** — how much better the data pin down some
  parameter combinations than others. Inverting the Hessian gives the
  covariance of the estimates; standardising it by the standard errors
  gives their correlation, whose eigenvalues are variances along
  directions in parameter space. The condition number is the largest
  over the smallest. This is the multi-parameter version of a 0.999
  pairwise correlation, and catches confounding spread over many
  parameters at once. On a fishery it usually means F traded against
  selectivity: scale F up, shift selectivity down, and predicted
  catch-at-age barely moves.

  The message leads with the square root, a ratio of standard errors.
  Because the matrix is scaled by those errors, the ratio is measured
  against each parameter’s own: 13,634 means the least-determined
  combination is 13,634 times more uncertain than the best-determined
  one, in those units. `WARN` above 1e6, `FAIL` above 1e10.

  Read on the **correlation** and not the covariance, because the
  covariance condition number is not scale-invariant: `log_F` sits near
  -2 and `sel_inf` near 10, so rescaling a parameter would move it
  without changing the model. Standardising costs sensitivity — the same
  three models read 4.6e4, 1.9e5 and 2.5e9 on the covariance against
  2.0e4, 1.6e4 and 1.9e8 here — so a model that is merely badly scaled
  no longer trips the check. The covariance number is still reported, as
  `covariance_condition_number`, for the numerical cost of inverting the
  Hessian; `pdHess` and `sdreport_failed` are what catch an inversion
  that actually fails. The thresholds were held at 1e6/1e10 on three
  models: `BS2017SS` sits at 2.0e4, roughly 1.7 orders below `WARN`.

  The parameters carrying the top 90% of each block’s loading are then
  named by fleet, bin and year, with a count: a ridge over ten years and
  one over a single year read differently. Complements
  [`TMBhelper::check_estimability`](https://rdrr.io/pkg/TMBhelper/man/check_estimability.html),
  which gives a per-parameter verdict, with a continuous severity and a
  direction.
...
### Naming the parameter, not just the block

TMB names every element of `obj$par` after its parameter block, so on
their own `estimability`, `parameters_on_bounds` and
`hessian_conditioning` could say `sel_coff_dev` was flagged but not
which of its 392 elements. Each now reports the fleet, sex, bin and year
instead, from
...
The three checks do not all count positions in the same vector.
`estimability` and `parameters_on_bounds` are captured from the hindcast
optimization; `hessian_conditioning` reads the `sdreport`, which under
an estimating HCR belongs to the projection fit and holds only
`log_Ftarget` / `log_Flimit`. Each check therefore verifies that the
index describes the vector it is about to label, and prints block names
alone when it does not — a coordinate naming the wrong parameter is
worse than no coordinate.

[`parameter_index()`](https://grantdadams.github.io/Rceattle/reference/parameter_index.md)
itself describes `fit$obj`, which under any HCR but `"NoFishing"` is
that projection object. Pass a fit run with `estimateMode = "Hindcast"`
to browse the hindcast parameters directly.
...
draws composition Pearson-residual bubbles and overlays the observed
(shaded area) and predicted (line) age or length compositions for every
fleet, both annually and aggregated across years. Joint-sex data are
drawn on a single bin axis with females above and males below zero.
...
plots the residuals as observed minus predicted, on whichever scale the
fleet is fitted on (log for a `Lognormal` fleet, the natural scale for
the others) — a useful first check for time trends or
heteroscedasticity. A positive residual means the survey saw more than
the model predicted. These return `ggplot` objects.
...
correlation across years. They are at least standardized by the variance
the fleet’s own likelihood assumes – the composition weight enters as an
effective sample size under a multinomial, and a Dirichlet-multinomial’s
overdispersion is in the denominator – so a weighted or DM fleet is not
inflated on that account alone. One-step-ahead (OSA) residuals remove
both problems. Each observation is residualized conditional on the
...
1.  **Inspect the aggregate fit** across years (`plot_comp(model_1)`):
    systematic lack of fit here points to structural misspecification
    (e.g. the wrong selectivity shape). Counts are pooled across the
    fitted hindcast years, and the interval holds 95% of the data the
    model predicts – not a confidence interval on the mean – so
    observations outside it are the ones to explain. The panel is
    annotated with the input sample size, the effective sample size the
    likelihood assumed (`ESS (likelihood)`), and the one this fit’s
    residuals imply (`ESS (McAllister-Ianelli)`). The gap between those
    two is the reweighting decision: they agree when the fleet is
    already weighted the way its residuals suggest.
2.  **Compare the SDNR** to its null interval (from
    [`osa_diagnostics()`](https://grantdadams.github.io/Rceattle/reference/osa_diagnostics.md)):
    a value outside the interval flags over- or under-dispersion, often
    a data-weighting or effective-sample-size issue.
3.  **Compare the lower/upper tail statistics** to their null intervals:
    these localize departures to the tails of the residual distribution.
    They are reported as order statistics – `lower_r` / `upper_r` name
    which one, and `lower_p` / `upper_p` the nominal probability it sits
    at, which is near but not exactly 2.5% – against the exact
    `Beta(r, n - r + 1)` null. The statistic and its interval must come
    from the same estimator, so
    [`osa_diagnostics()`](https://grantdadams.github.io/Rceattle/reference/osa_diagnostics.md)
    no longer simulates and its `nsim` / `seed` arguments are ignored.
    The residuals themselves remain randomized-quantile residuals: pass
    a seed to
    [`osa_residuals()`](https://grantdadams.github.io/Rceattle/reference/osa_residuals.md)
    to control those.
4.  **Inspect Pearson bubble plots** (`plot_comp(model_1)`, the
    default): patterns in the sign of residuals across age/year can
    identify *where* lack of fit occurs – Stewart and Monnahan recommend
    keeping these alongside OSA residuals.
5.  **Revise the model or data weighting** where misspecification is
...
Restrict it to particular fleets with `fleets =`, taking either
`Fleet_code` values or names matching `fleet_control$Fleet_name` — give
ids or names, not a mix, since R coerces `c(1, "Survey")` to character.
...
estimates its own weight inside the likelihood, so tuning it externally
would compete with that estimate; those fleets are named and skipped.
...
iteration entirely — note that under that likelihood `Comp_weights` is
read as the **log** of the starting weight, so the model default of `1`
is a starting weight of *e*, and a weight of 1 is written as `0`.

Because OSA residuals for discrete composition data are randomized
quantile residuals, they are stochastic;
[`osa_residuals()`](https://grantdadams.github.io/Rceattle/reference/osa_residuals.md)
takes a `seed` for reproducibility.
...
both fit peels / starts in parallel by default. Pass `cores = 1` to
force sequential execution; otherwise the call uses a PSOCK cluster
sized at `parallel::detectCores() - 6` (capped at 2 under
`R CMD check`).
...
Each simulation uses `seed + i` as its RNG seed, so results are
reproducible whether or not `cores > 1`. Set `simulate = FALSE` to refit
against the model’s *expected* values (no observation error) rather than
draws from the observation likelihood — useful for confirming that the
estimator returns the generating parameters in the noise-free limit.
...
CV) understates the truth. By default only the observations are redrawn:
every replicate shares the operating model’s single realization of
recruitment and mortality, so the spread across replicates is
observation error alone — a lower bound on estimation uncertainty in SSB
and recruitment, not comparable to the model’s own uncertainty bands.

Third, watch for a truncation warning on a natural-scale index. A fleet
with `Index_distribution = "Normal"`, `"MVN"` or `"MVNORM"` is drawn on
the natural scale with an absolute sd, so a draw can come out negative —
impossible for an index, and rejected by
...
*truncated at zero* rather than the untruncated normal the likelihood
maximises. Occasional truncation is harmless; a lot of it is not. An
acoustic index observed at 0.70 with an absolute sd of 0.80 rejects a
fifth of its draws and the truncated mean sits 39% above the untruncated
one, so the self-test is measuring recovery against a data-generating
process the estimator never assumed.
[`sim_mod()`](https://grantdadams.github.io/Rceattle/reference/sim_mod.md)
warns when any row puts more than 2% of its probability at or below zero
— `Phi(-mu/sd)`, read off the fit rather than counted from the draw, so
it does not vary between replicates.

The fix for a `Normal` fleet is
`Index_distribution = "TruncatedNormal"`, which fits the truncated
density — renormalized by `log Φ(µ/σ)` over the `(0, ∞)` the data can
actually occupy — and draws from it by inverse CDF. Draw and likelihood
then agree exactly and no draw is ever rejected. `"Normal"` is kept
untruncated because it reproduces the AMAK `avo_like` / `cpue_like` term
for term, which is what an ADMB bridge compares against; use it there
and `"TruncatedNormal"` elsewhere. `MVN`/`MVNORM` have no such option: a
multivariate normal truncated to the positive orthant has no closed-form
sampler, so rejection is the only route and the gap stays. Either way
the underlying issue is the same — an absolute sd that large relative to
the index means the natural-scale normal was a poor observation model to
begin with.

`"TruncatedNormal"` OSA residuals need one thing said about them. The
truncation constant is a function of the prediction, not of the
observation, so it drops out of any method that reads the curvature of
the density in the observation — including `oneStepPredict()`’s default
`oneStepGaussianOffMode`, which would return the untruncated
`(obs - µ)/σ`.
[`osa_residuals()`](https://grantdadams.github.io/Rceattle/reference/osa_residuals.md)
therefore residualizes this family in its own call, integrating the
density over `(0, ∞)`, so the residual is `qnorm(F(x))` for the
truncated `F`. The correction is the size of the truncated mass: on a
fleet predicting 100 with an absolute sd of 150, an observation exactly
at the prediction moves from a residual of 0 to −0.44. You do not need
to pass anything for this; `method` is overridden for those rows only.

### Redrawing process error

`process` widens the question. The default (`FALSE`) asks whether the
estimator recovers its own parameters from new observations; naming a
process redraws that process too, and asks whether it recovers something
it was not shown.
...
`styr`. Natural mortality covers the `M1_re` random effects, and growth,
catchability and selectivity are redrawn wherever they are written as a
random linkage (see
...
A process the model gives no distribution to — asking for M when
`M1_re = 0`, say — is left alone and reported in a warning rather than
silently returned unchanged.

Recruitment has one such case worth knowing about. Under `srr_fun = 0`
with `srr_pred_fun > 0` — the AMAK/Ianelli configuration, where
recruitment is estimated annually and the stock-recruit curve is fitted
as a *penalty* on those same deviations — the deviations are scored by
two densities at once.

Two terms on one quantity do not compose into a distribution to draw
from. Drawing at `R_sd` from the first alone ignores the second, and the
second is not a density on the deviations alone either: it couples them
to the stock-recruit parameters, which are estimated too. Either way the
self-test would measure recovery against a process the model does not
assume, so the deviations are not redrawn and
[`sim_mod()`](https://grantdadams.github.io/Rceattle/reference/sim_mod.md)
says why. A random linkage on a recruitment parameter is a separate
latent and is still drawn. Set `srr_fun` to the stock-recruit function
itself to redraw the deviations.

The fitted deviations are no longer what generated the data, so the
values that *did* come back attached to each replicate, each with a mask
of the cells the draw touched:
...
mask matters: the process draws cover the hindcast only, and
`beta_linkage_re` spans every random linkage whether or not its process
was asked for, so a correlation taken over the whole array scores fitted
values as though they had been simulated. For the same reason
[`compare_sim()`](https://grantdadams.github.io/Rceattle/reference/compare_sim.md),
which compares against the operating model, is not valid for
`process`-drawn replicates.

One caveat before reading a redrawn M as validation. On `BS2017SS` with
a year-varying M random effect, refits recover the simulated deviations
(correlation around 0.5, positive in every replicate) but recover their
standard deviation poorly — roughly 70% low, and collapsing to zero in a
third of replicates. That is the usual behaviour of a variance component
estimated from little information, not a fault in the simulation, and a
year-varying M is weakly informed because it trades off against
selectivity and catchability. Read a self-test that redraws M as a test
of deviation recovery rather than of variance recovery.
...
### Moving a set of cells together

`slots` cross, so *k* slots give a *k*-dimensional profile. That is
wrong for a parameter supplied as a vector — an empirical age-based M
profiled over 13 values across ten ages would be `13^10` fits. `joint`
puts every named cell on **one** grid instead: `"multiply"` scales each
cell’s current value, preserving the shape of the schedule (`1` is the
fitted model), and `"add"` shifts it, preserving the increments (`0` is
the fitted model).
...
prof_M_all <- profile(
  fitted = model_1,
  param  = "M1",
  slots  = lapply(seq_len(model_1$data_list$nages[sp]),
                  function(a) c(sp, 1, a)),   # every age bin of species 1
  values = list(seq(0.7, 1.3, by = 0.05)),
  joint  = "multiply"
)

plot_profile(prof_M_all)
```

### Catchability

`param = "q"` profiles a fleet’s base catchability, and `slots` takes a
fleet name. Fleets sharing a `Catchability_index` share one q, so naming
any member profiles the whole group — the others are added with a
message.
...
`grid` (the user-scale value grid — a multiplier or offset under
`joint`), `nll` (joint NLL aligned with `grid`, `NA` for non-converged
fits), `alias` (the natural-scale name you profiled under, if you used
one), `joint`, and echoes of `param` and `slots`. To cross-profile
across multiple species, supply one slot per species
...
When one component dwarfs the rest the others flatten onto the axis and
where they prefer the parameter cannot be read. `relative = "scaled"`
puts every curve on 0 to 1 so the minima can be compared directly; it
discards magnitude, so raise `minfraction` alongside it.
