...
across a grid of fixed values for one or more cells of a chosen
parameter and returns the resulting NLL surface. A flat profile means
the data carry little information about the parameter; a sharp minimum
away from the MLE means the fit has not actually settled there. The
function generalises the legacy `profile_rsigma()` helper from
