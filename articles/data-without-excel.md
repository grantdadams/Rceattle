...
by simply not setting the field on the list. The single-species example
below shows the full machinery, but feel free to skip sections you don’t
need.
...
This is the most important configuration table. Each row defines one
fleet (fishery or survey) and controls how it is modelled. Multiple
fleets can target the same species.
...
the log-scale biomass index (or numbers if `Weight1_Numbers2 = 2`). A
...
[`read_data()`](https://grantdadams.github.io/Rceattle/reference/read_data.md).
This makes it easy to share data objects or move between R-based and
