...
normal directly on the log scale; under the default
`fit_control(bias_adjust_proc = TRUE)` it is centred at
`meanlog - sdlog^2/2`, so `exp(meanlog)` is the prior mean (the median
with `FALSE`). On covariate rows the prior applies to the coefficient
as-is, regardless of link function.
...
An `M1` intercept prior replaces `build_M1(M1_use_prior = TRUE)` for
that species, and
[`fit_mod()`](https://grantdadams.github.io/Rceattle/reference/fit_mod.md)
refuses the pair unless `M2_use_prior = TRUE` (a prior on total M).
...
The linkage prior is applied verbatim, with no lognormal bias-correction
term.
...
- a **two-level nested list keyed first by specified species, then by
  specified sex id**.
...
RHS, another with `sex = 2L` and a different RHS.
...
- **`R0`** – acts under mean recruitment (`srr_fun = "mean"`, penalty
  ...
  without fitting an SRR. Under a curve fitted in the hindcast a
  single-species `R0` is derived from alpha and beta, so an `R0` linkage
  is refused. In a multispecies model `R0` sets only the initial level
  (`R_init`), so only an intercept-only linkage (`~ 1`, a prior or fixed
...
### Priors and fixed values on R0, alpha and beta

An intercept-only formula (`~ 1`) acts on the parameter itself, so the
same grammar gives a prior or a fixed value. This is the recommended
alternative to `srr_est_mode` / `srr_prior`:

``` r

# Lognormal prior on alpha (mean 5, log-scale SD 0.5), species 1 only.
rec_prior <- build_srr(
...
# Alpha fixed at 20: an intercept held at its `init`.
rec_fixed <- build_srr(
  srr_fun  = "BevertonHolt",
  linkages = list(
    alpha = linkage_spec(~ 1, init = list(`(Intercept)` = 20), est_phase = 0)
  )
)
```

`prior_lognormal(p1, p2)` is a normal density on the log of the
parameter, centred so `p1` is the log of its mean under the default
`fit_control(bias_adjust_proc = TRUE)`, and of its median with `FALSE`.
For a Ricker curve it gives the same objective as
`srr_est_mode = "LognormalPrior"`, which is deprecated (5.33.0); using
both is refused. The one prior a linkage cannot express is on
Beverton-Holt steepness, which needs spawning biomass per recruit:
`srr_est_mode = "LognormalPrior"` or `"BetaPrior"` remains for it, in
single-species models only.
...
prior *and* link `q` to a covariate. This replaces
`Catchability = "Estimated-with-prior"`: both put a prior on the same
`q`, so
[`fit_mod()`](https://grantdadams.github.io/Rceattle/reference/fit_mod.md)
refuses the pair, including on a `q` shared through
`Catchability_index`.
...
keeps the optimizer grounded. With `est_phase = 0` the base parameter is
held at `X` even when `fit_mod(inits = )` carries another value; for an
...
- **Only the grammar’s random effects are simulated.**
  `sim_mod(process = )` and `self_test(process = )` redraw catchability
  and selectivity where they are written as a random linkage; the legacy
  `Time_varying_q` / `Time_varying_sel` deviations carry densities in
  the model but no draw, so they keep their fitted values and
  [`sim_mod()`](https://grantdadams.github.io/Rceattle/reference/sim_mod.md)
  warns that they did. Recruitment and M are drawn from their own
  switches (`M1_re`) as well as from linkages. Observed AR1 (Rogers
  QAR1) linkages are the one grammar form that is deliberately not
  redrawn: the latent state is measured by an observed covariate series,
  and redrawing the state alone would leave the two describing different
  histories. A switch-based time-varying process therefore cannot have
  that process redrawn in a self-test.
- **`sigma` means different things in the two systems.** In a linkage,
  `init = list(sigma = )` under `ar1()` is the **marginal** standard
  deviation. The `M1_re` and 2DAR1/3DAR1 selectivity switches instead
  take the AR(1) **innovation** SD, and scale it to a marginal of
  `sigma / sqrt(1 - rho^2)` — the WHAM convention. The two coincide only
  at `rho = 0`.
...
- **Legacy retirement** – `srr_fun %in% c(1, 3, 5)` and `srr_indices`
  are errors from 5.32.0: express the environmental effect as a
  recruitment linkage. `M1_indices` and `M1_model %in% c(4, 5)` remain
