...
appears in any linkage row, that base parameter is automatically mapped
out (held NA in the TMB map) so the linkage entirely replaces its role –
see *Automatic base-parameter handling* below.
...
| `~ 0 + temp` (slope only) | left at [`build_params()`](https://grantdadams.github.io/Rceattle/reference/build_params.md) default | NA (not estimated) |
...
base array slot is held at `0` so the model evaluates
`(Intercept) + (slopes)` cleanly, with no hidden offset.

Slope-only formulas (`~ 0 + temp`) do *not* identify the level. The base
parameter is still mapped out (so it cannot drift), but it stays at the
[`build_params()`](https://grantdadams.github.io/Rceattle/reference/build_params.md)
default (e.g. `log_R0 = 9`). If you need a non-default level, supply it
via `inits`, or add an explicit intercept to the formula.
...
`by = ~ species + sex` zeros each (species, sex) combination present in
the linkage table.
...
bounds, or `init` if you need to constrain the parameter.

### Single-sex models and `by = ~ ... + sex`

If your model is single-sex (`nsex = 1` for every species in scope),
asking for `by = ~ species + sex` produces a warning: there is only one
sex level available, so the sex stratum collapses to a single shared
level. The result is equivalent to `by = ~ species`. Models with mixed
`nsex` (some single-sex, some two-sex species) emit one sex level per
...
- **`formula`** – a one-sided R formula whose RHS describes the linear
...
- **`by`** – a one-sided formula naming the stratifying factors that
  ...
  per species, the typical multispecies assessment use case). Pass
  `~species + sex` for per-(species, sex) coefficients,
  `~species + age_bin` for an age-binned M response, or `NULL` to share
  a single coefficient set across every species/sex.
...
Inside `linkage_spec(priors = ...)` the same constructors are available
...
Each prior’s negative log-density contributes to slot 19 of the joint
NLL (`fit$quantities$jnll_comp[20, ]` in R, since R is 1-indexed):
...
environmental covariates, you want to apply a Bayesian-style informative
prior directly to the base value of a parameter like `log_K`, `log_R0`,
or `log_M1`.
...
*is* the parameter, and a prior on its `(Intercept)` row is the prior on
that parameter:
...
`build_M1(M_prior = ..., M_prior_sd = ..., M1_use_prior = TRUE)`
mechanism, the linkage-intercept route gives you the same prior shape
but composes naturally with the rest of the linkage table (you can keep
your env-effect rows and the intercept rows together, with priors on
each). For most operational work the existing `M_prior` mechanism is
simpler and more direct; the linkage route is most useful when you
already have a table of env-effect coefficients and want to extend the
same per-row prior/bound machinery to the parameter intercept.

## Species- and (species, sex)-specific priors

Each entry under `priors = list(...)` may be:

- a single prior – applies to every species/sex row that uses that
  design column,
- a **named list of priors keyed by species id** – one prior per
  species, or
- a **two-level nested list keyed first by species, then by sex id** –
  one prior per (species, sex).
...
For sexually dimorphic stocks, the same `temp` column can take different
priors per (species, sex). At each level the value can be either an
`Rceattle_prior` (applies to all sub-strata) or a list that drills down
further:
...
The same syntax works for any prior family. This is the recommended
pattern when a covariate has different prior strength across stocks
(e.g. when one species has prior literature on its temp sensitivity but
another doesn’t), or when within a species the sexes are expected to
respond differently. Built programmatically, it’s just:
...
Validation: at each level the value must be either an `Rceattle_prior`
*or* a named list of priors keyed by the next stratum. Non-prior values
(numeric vectors, mismatched classes) are caught at
[`linkage_spec()`](https://grantdadams.github.io/Rceattle/reference/linkage_spec.md)
time with a pointer to the offending (column, species\[, sex\]) path.

## Per-species formulas

If species have qualitatively different env relationships – e.g. species
1 responds only to temperature, species 2 also to PDO – register
*multiple specs against the same parameter* and use the `species`
argument on each to scope it to a subset of stocks:
...
accepts both forms under each parameter key:
...
`Rceattle_linkage_spec` (single form) or a `list` of them (multi-spec
form), and `fit$data_list$linkage_table` shows the materialized rows
with `species` and `X_col` correctly scoped.

This pattern composes with species-specific *priors* – each spec carries
its own priors, and within a spec the priors can themselves be
species-keyed if you want even finer control:
...
The `sex` argument on
[`linkage_spec()`](https://grantdadams.github.io/Rceattle/reference/linkage_spec.md)
mirrors `species`: it scopes a spec to a subset of sex ids. Combined
...
sex-dimorphic M priors or distinct env responses for females vs. males:
...
want different *formulas* per sex (rather than just different priors or
inits), use the multi-spec form – one spec with `sex = 1L` and its own
RHS, another with `sex = 2L` and a different RHS. The pooler unions the
design columns by name in the same way it does for per-species formulas.

If you only need different priors per (species, sex) for a single shared
formula, the nested-priors form (see *Species- and (species,
sex)-specific priors* above) is simpler – one spec, one set of design
columns, prior values keyed by `species` then `sex`. Reach for the
per-sex multi-spec form when you need different RHS terms, different
`init`/`bounds`, or different `est_phase` between sexes.
...
so any expression
[`model.matrix()`](https://rdrr.io/r/stats/model.matrix.html)
understands works – polynomial bases, splines, interactions, and
arithmetic transformations.

**Polynomials.** Use [`poly()`](https://rdrr.io/r/stats/poly.html) for
orthogonal polynomial bases (the default; numerically well-conditioned),
or `poly(..., raw = TRUE)` for raw monomials. Each basis order shows up
as a separate design column with its own coefficient:
...
The design column name is the deparsed call (`I(temp^2)`), so when you
key `init`, `bounds`, or `priors` against it you have to use the same
string – backticks let you write it as a name.
...
**Interactions.** `temp:PDO` adds a single product column; `temp * PDO`
is shorthand for `temp + PDO + temp:PDO`:
...
**Mixing forms across species.** Combined with the per-species formula
machinery, you can give one species a polynomial response and another a
linear one without duplicating columns:
...
across specs that use them (this is enforced –
[`pool_linkages()`](https://grantdadams.github.io/Rceattle/reference/pool_linkages.md)
errors loudly on mismatches). With deterministic formulas evaluated
...
5.  **The TMB template** iterates the linkage rows once and builds
    ...
    - `recruitment_linkage_offset` of shape `[nspp, n_rec_params, nyrs]`
      via `rceattle_apply_recruitment_linkages()`, pre-added to
      `(R0, rec_pars(*, alpha), rec_pars(*, beta))` before each
      `calculate_recruitment()` call site (hindcast, BRPs, dynamic BRPs,
      projections, expected R).
...
The structural M choices (`M1_model`) and the random-effects choices
(`M1_re`) accept either integer codes or readable strings
interchangeably:
...
`linkages` on M is age-aware: by default a row’s `age_bin` is `NA` and
the offset broadcasts across every age. To pin an env effect to a
specific age slice, set `by = ~ species + age_bin` and supply
...
linkage rows for both processes, and the underlying coefficient vector
`beta_linkage` indexes both.
...
Practical note: a linkage row’s `age_bin` is ignored for recruitment
...
[`fit_mod()`](https://grantdadams.github.io/Rceattle/reference/fit_mod.md)
pools them against a single shared design matrix:
...
- **Legacy retirement (v4.2.0)** – `srr_indices`, `M1_indices`,
  ...
  minor release; see *Scheduled removal (v4.2.0)* in `NEWS.md` for the
  migration table.
