...
table cooperate cleanly: the **base parameter holds the level**, and the
**linkage rows hold year-by-year offsets** that multiply the base on the
log scale. The exact behaviour depends on whether the formula carries an
intercept:

| Formula | Base parameter | `(Intercept)` row | Slope rows |
|----|----|----|----|
| `~ 1` (intercept only) | estimable; init from spec or default | fixed at `0` (NA in TMB map) | n/a |
| `~ temp` (intercept + slope) | estimable; init from spec or default | fixed at `0` (NA in TMB map) | estimated |
| `~ 0 + temp` (slope only) | mapped NA, fixed at [`build_params()`](https://grantdadams.github.io/Rceattle/reference/build_params.md) default | n/a (no intercept row produced) | estimated |

**Intercept-bearing formulas (`~ 1`, `~ temp`)** – the base parameter
remains estimable and carries the level; for example `log_R0` for
species `s` lives on `rec_pars[s, 1]` exactly as it does without any
linkages. The `(Intercept)` row of the table exists for bookkeeping and
as a hook for `init`/`prior` (see below) but its `beta_linkage` slot is
held at `0` and not estimated. The recruitment / mortality / growth
modules read `base + offset(yr)` each year – with the intercept fixed at
zero, the offset is exactly the slope contribution.

**Slope-only formulas (`~ 0 + temp`)** – there is no `(Intercept)` row,
so the base parameter is masked out (NA) and stays at the
[`build_params()`](https://grantdadams.github.io/Rceattle/reference/build_params.md)
default (e.g. `log_R0 = 9`). If you need a different level, either
supply it via the model’s `inits` argument or add an explicit intercept
to the formula and set `init = list(`(Intercept)`= ...)` on the spec.

### `init` and the base parameter

`init = list(`(Intercept)`= X)` on the linkage spec **flows to the base
parameter** rather than to the linkage row. The linkage row’s
`beta_linkage` slot stays at `0`; the base parameter starts at `X`. This
...
`by = NULL` (a fully shared linkage) produces a single shared row; and
...
parameter**: the slot 19 contribution evaluates the prior density
against `rec_pars`, `log_M1`, or `log_growth_pars` (whichever the row
points at), not against the (zeroed) `beta_linkage` value. The user sees
the same syntax they would use for any other column, and the prior
actually constrains the parameter that carries the level.
...
You can inspect the linked parameter values via the linkage table, the
estimated slope coefficients, and the base-parameter array:
...
each). One small difference from the legacy `M_prior` path: the linkage
prior is applied verbatim, with no lognormal bias-correction term added
internally. If you want a literal port of the old `M_prior` behaviour,
use `normal(log(M_mean) + sd^2 / 2, sd)` so the mean of the
natural-scale M matches `M_mean`.
