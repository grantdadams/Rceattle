...
These are built with **ggplot2** (colourblind-safe viridis palette,
`theme_classic`) and **return the `ggplot` object**, so they print when
called interactively and can be further customised by adding ggplot2
layers (e.g. `plot_biomass(fit) + ggplot2::ggtitle("My title")`). Pass
