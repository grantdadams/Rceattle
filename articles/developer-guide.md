...
5.  Add it to the parameter dictionary in `R/0-parameter_dictionary.R`.
    It is a registry keyed by parameter name, so a new (or retired)
    block that misses it fails only later.
6.  If the parameter is a random effect, give it a `SIMULATE` draw
    beside its density (see *Change the likelihood*), or
...
- **Report under a `*_sim` name.** TMB never clears its report
  ...
  visible in `obj$report()` afterwards. A draw written into the
  observed-data object and reported under its name is readable as the
...
The diagnostics take the fitted model as `object` (5.11.0). `Rceattle`
and `fit` still work and are silent; they warn from 5.12.0 and go in
6.0.0. To find your own call sites now:
...
Three keep their own names on purpose.
[`profile()`](https://rdrr.io/r/stats/profile.html) takes `fitted`
because it is an S3 method for
[`stats::profile()`](https://rdrr.io/r/stats/profile.html), and renaming
a method’s first formal away from its generic fails `R CMD check`.
`mse_summary(mse = )` and `compare_sim(operating_mod = )` take an MSE
result and a set of simulations, not a fitted model, so their names say
more than `object` would.

The rest of the surface below is **known-awkward and deliberately
unchanged**. It is recorded here so it is not reopened each time someone
reads the API cold.
...
- `estimateMode` 0 = “Estimate” but 1 = “Hindcast”, which reads
  backwards.
...
is the one place where `object` means something else.
