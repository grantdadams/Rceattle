...
b2 <- microbenchmark({obj <- MakeADFun(data, parameters, random=c("U"), DLL="sam", silent = TRUE)})
...
o2 <- microbenchmark({opt <- nlminb(obj$par, obj$fn, obj$gr, lower=lower, upper=upper)})
r2 <- microbenchmark({rep <- sdreport(obj)})

f2 <- microbenchmark({obj$fn()})
g2 <- microbenchmark({obj$gr()})

df <- data.frame(Model = "SAM", Function = c("MakeADFun", "Fit", "Sdreport", "fn", "gr"), TMB = round(sapply(list(b2,o1,r2,f2,g2), function(x) mean(x$time)) * 1e-9, 4), RTMB = round(sapply(list(b1,o1,r1,f1,g1), function(x) mean(x$time)) * 1e-9, 4))
