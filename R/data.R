...
#' \item{Catchability_init}{Starting value (or fixed value) for catchability. When Catchability = "Estimated-with-prior" this also centres the q prior.}
...
#' \item{Comp_weights}{Composition weight. For a multinomial family it multiplies the input sample size; for a Dirichlet-multinomial it is the starting value of the estimated weight. Either way it is the value used when a model is built from scratch -- a fit warm-started from \code{inits} carries the parameter forward. A fitted model reports the implied McAllister & Ianelli (1997) harmonic-mean weight in \code{Comp_weights_mcallister}; \code{\link{reweight_comps}} iterates on it.}
