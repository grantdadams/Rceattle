...
#' \item{Catchability_index}{Index used to give fleets the same catchability. Fleets sharing a value share one q parameter, so the group can carry only one answer to whether q is estimated: the lead fleet (the group's first non-Off fleet) decides for all of them, regardless of fleet type. A fleet with no index_data of its own is therefore still estimated if its group's lead is; give a fleet its own value when it should be estimated independently. The exception is "Analytical" and "AnalyticalArith", which solve catchability from each fleet's own index observations rather than from the shared parameter, so a group containing one does \emph{not} share a catchability even when every fleet in it carries the same setting. "Environmental" and "AR1" do share: they rebuild q from the group's shared parameters, including when the fleets name different environmental series (the lead fleet's is used). If the lead is "Fixed" the group shares nothing at all: no catchability is estimated and each fleet sits at its own \code{Catchability_init}. Where a q \emph{is} estimated for the group, its starting value is the mean of the group's log \code{Catchability_init} (TMB averages the initial values of parameters mapped together), not the lead's; the prior centre does use the lead's. \code{data_check()} reports the analytical case, a Fixed lead that leaves fleets on different inits, and a differing \code{Catchability} or \code{Time_varying_q} within a group.}
#' \item{Catchability}{Estimate catchability? 0 or "Fixed" = fixed at prior; 1 or "Estimated" = estimate a single parameter; 2 or "Estimated-with-prior" = estimate a single parameter with a prior; 3 or "Analytical" = geometric-mean analytical q (Ludwig and Walters 1994); 4 = power equation (not yet implemented); 5 or "Environmental" = linear equation log(q_y) = q_mu + beta * index_y; 6 or "AR1" = removed, and \code{data_check()} errors on it -- annual AR1 catchability deviates fit to an environmental index (Rogers et al. 2024), except that the deviates were never estimated, so q came back constant; use a q linkage with ar1(1 | Year) and \code{observe}, passed to \code{fit_mod(qFun = )}, instead; 7 or "AnalyticalArith" = arithmetic-mean analytical q = mean(obs)/mean(pred), the AMAK/ebswp form used with the MVN survey likelihood.}
...
#' \item{Time_varying_q}{Whether a time-varying q should be estimated. 0 = "Off", 1 = "IID" penalized deviate or random effect, 2 = "AR1" REMOVED in 5.16.0 (it was never an AR1 -- the template scored it with the same independent normal penalty as "IID", and \code{index_q_rho} belongs to the removed \code{Catchability = "AR1"} path -- so \code{data_check()} errors on it; express an autocorrelated deviation as a q linkage, \code{linkage_spec(~ ar1(1 | Year))}), 3 = "Block" time blocks with no penalty; 4 = "RandomWalk" random walk from mean following Dorn 2018 (dnorm(q_y - q_y-1, 0, sigma). If Catchability = 5, this determines the environmental index to be used in the equation log(q_y) = q_mu + beta * index_y}
