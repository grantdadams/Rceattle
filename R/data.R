...
#' \item{Comp_weights}{Composition weight. Under a multinomial likelihood it multiplies the input sample size; under a Dirichlet-multinomial it starts the estimated weight. It is read when a model is built from scratch, so a refit keeps whatever weight it is given. A fit reports the weight its residuals imply in \code{Comp_weights_mcallister}; \code{\link{reweight_comps}} tunes towards it.}
