...
#' \item{ssb_wt_index}{Weight-at-age (weight) index to use for calculation of each species spawning biomass}
...
#' \item{M1_model}{Estimate residual (multi-species mode) or total natural mortality (single-species mode). 0 = use fixed natural mortality from M1_base, 1 = estimate sex- and age-invariant M1, 2 = sex-specific (two-sex model), age-invariant M1, 3 = estimate sex- and age-specific M1. (Formerly named \code{est_M1}; the old name is still accepted with a deprecation message.)}
...
#' \item{env_data}{Environmental indices such as bottom temperature data to incorporate into ration equation specificed by Ceq and Cindex. Also used to drive catchability if Catchability = 5. Will use the mean for missing years. Temperature should be in celcius.}
...
#' \item{Selectivity}{Selectivity to use for the species: 0 = "Fixed"; 1 = "Logistic"; 2 = "NonParametric" sensu Ianelli et al 2018; 3 = "DoubleLogistic"; 4 = "DescendingLogistic"; 5 = "Hake" non-parametric sensu Taylor et al; 6 = "2DAR1"; 7 = "3DAR1" sensu Cheng et al 2024; 8 = "DoubleNormal" (Gaussian ascending and descending limbs blended at a common peak — similar to SS3 pattern 24). Parameters: `sel_inf[1]` = peak bin/length; `sel_inf[2]` = logit(right_floor), right-tail floor analogous to SS3 P6/end_logit (logit=-10 → dome-shaped, logit=+10 → logistic ascending-only); `log_sel_slp[1]` = log(sigma_ascending); `log_sel_slp[2]` = log(sigma_descending). Supports the same time-varying modes as DoubleLogistic (IID, Block, RandomWalk).}
#' \item{Selectivity_dimension}{"Age" or "Length".}
#' \item{N_sel_bins}{Number of bins (ages or length bins) to estimate for non-parametric selectivity. (Formerly \code{Nselages}; the old name is renamed on read.)}
...
#' \item{Sel_shape_sd, Sel_curvature_sd, Sel_devmag_sd}{Non-parametric selectivity penalties expressed as standard deviations (an intuitive alternative to the raw penalty weights in Sel_curve_pen1/2/3). Each penalty is a Gaussian SSQ, so the weight is 1/(2*sd^2): Sel_shape_sd is the shape/smoothness penalty, Sel_curvature_sd the 2nd-difference curvature penalty, Sel_devmag_sd the deviation-magnitude penalty. Supplying an SD sets the corresponding Sel_curve_pen column; a legacy Sel_curve_pen value is never overwritten. Applies to the non-parametric penalty-weight forms only (LogisticPM and 2D/3D-AR1 keep the Sel_curve_pen columns).}
#' \item{Sel_shape_dir}{Direction of the (directional-mode) non-parametric shape penalty when Sel_shape_sd is used: "Decreasing" (default, penalize decreasing selectivity) or "Increasing". Sets the sign of Sel_curve_pen1.}
...
#' \item{Q_init}{Starting value (or fixed value) for catchability. When Catchability = "Estimated-with-prior" this also centres the q prior. (Formerly \code{Q_prior}; the old name is still accepted with a deprecation message.)}
#' \item{Q_sd_prior}{Standard deviation of the q prior: dnorm(log_q, log_q_prior, q_sd_prior). Used only when Catchability = "Estimated-with-prior".}
...
#' \item{Estimate_index_sd}{Estimate survey/index observation SD. Accepts integer codes or readable strings: 0 / "Fixed" = use the fixed SD from the data CV; 1 / "Estimated" = estimate; 2 / "Analytical" = analytically estimate following Ludwig and Walters 1994. (Formerly \code{Estimate_survey_sd}; renamed on read.)}
#' \item{Index_sd}{Starting/fixed value for the index observation SD, used when Estimate_index_sd = "Estimated". (Formerly \code{Survey_sd_prior}, then \code{Index_sd_prior}; the old names are still accepted.)}
