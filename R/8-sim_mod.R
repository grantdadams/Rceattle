...
sim_mod <- function(ceattle_obj, simulate = FALSE) {
    ...
    for (obs in 1:nrow(dat_sim$srv_comp)) {
        ...
        if (simulate) {
            # Simulate FIXME add dirichlet multinomial
    ...
    if (length(dim(dat_sim$UobsWtAge)) == 4) {
        for (sp in 1:dat_sim$nspp) {
            for (r_age in 1:dat_sim$nages[sp]) {
                if (ceattle_obj$data_list$suitMode > 0 & simulate & sum(ceattle_obj$quantities$mn_UobsWtAge_hat[sp, , r_age, 
                  ] > 0) > 0) {
                  values <- rmultinom(n = 1, size = dat_sim$stom_tau[sp], prob = ceattle_obj$quantities$mn_UobsWtAge_hat[sp, 
    ...
    if (length(dim(dat_sim$UobsWtAge)) == 5) {
        for (sp in 1:dat_sim$nspp) {
            for (yr in 1:dat_sim$nyrs_fsh_comp[sp]) {
                for (r_age in 1:dat_sim$nages[sp]) {
                  if (ceattle_obj$data_list$suitMode > 0 & simulate & sum(ceattle_obj$quantities$UobsWtAge_hat[sp, , r_age, 
                    , yr] > 0) > 0) {
                    values <- rmultinom(n = 1, size = dat_sim$stom_tau[sp], prob = ceattle_obj$quantities$UobsWtAge_hat[sp, 
