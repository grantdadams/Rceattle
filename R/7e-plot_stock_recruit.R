...
plot_stock_recruit <-
  function(Rceattle,
    ...
    for (i in 1:length(Rceattle)) {
      ssb_array[,1:nyrshind[i],i] <- Rceattle[[i]]$quantities$biomassSSB[,1:nyrshind[i]]
      rec_array[,1:nyrshind[i],i] <- Rceattle[[i]]$quantities$R[,1:nyrshind[i]]
      ...
      for (j in 1:length(spp)) {
        ...
        for (k in 1:length(Rceattle)) {
          ...
          if(srr_pred_fun[k] %in% c(1,2)){
            curve(exp(rec_pars[spp[j], 2, k]) * x / (1+exp(rec_pars[spp[j], 3, k]) * x), 
          ...
          if(srr_pred_fun[k] %in% c(3,4)){
            curve(exp(rec_pars[spp[j], 2, k]) * x * exp(-exp(rec_pars[spp[j], 3, k]) * x/1000000), 
                  from = 0, to = xmax[spp[j]],
            lty = 1, lwd = lwd, col = line_col[k], 
            add = TRUE)
          }
