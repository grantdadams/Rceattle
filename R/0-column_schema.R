...
.rce_column_schema <- function() {
  rows <- list(
    ...
    .rce_col("Sel_norm_bin", "fleet_control", "Age/length bin at which selectivity = 1. An absolute AGE for an age-based fleet (6 means age 6, not the 6th bin) or a 1-based LENGTH-BIN ordinal for a length-based one, per 'Selectivity_dimension'. \r\nIf NA, it will not normalize selectivity. \r\nIf < 0, will normalize selectivity by the max. \r\n\r\nIn a two-sex model this column also sets how the sexes are scaled RELATIVE to each other, which is what decides whether relative sex-specific selectivity is estimated: \r\n< 0 normalizes by the maximum taken jointly across bins AND sexes, so one sex peaks at 1 and the other keeps its relative level (relative selectivity retained); \r\n>= 0 normalizes each sex separately at that bin, forcing both to 1 there, so only the SHAPE differs by sex (relative selectivity removed); \r\nNA does not normalize, leaving the relative scale free. \r\nRelative sex selectivity is only informed by the data where the composition is joint (comp_data Sex = 3); sex-specific rows (Sex = 1 or 2) each sum to 1 and carry no sex-ratio information. Selectivity = 'Hake' always normalizes within sex regardless of this column.", type = "integer", has_default = TRUE, default = NA, default_msg = "'Sel_norm_bin' not specified in 'fleet_control', assuming 'NA'", aliases = c("Age_max_selected", "Sel_norm_bin1")),
