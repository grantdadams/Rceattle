...
.rce_column_schema <- function() {
  rows <- list(
    ...
    .rce_col("Comp_weights", "fleet_control", "Starting composition weight, used when a model is built from scratch; a fit warm-started from `inits` carries the parameter forward instead. \r\nA fitted model reports the implied McAllister & Ianelli (1997) harmonic-mean weight separately, in `Comp_weights_mcallister`; `reweight_comps()` iterates on it."),
