...
## Climate data ----
summer_bt_data <- read.csv("~/GitHub/Rceattle_MSE/Data/goa_temp_610_to_630_summer_300M.csv") %>%
  filter(depthclass == "Bottom", hind == "yes")  %>%
  mutate(varname = "summer bt") %>%
  rename(value = mean_value_dc_610_to_630)

fall_sst_data <- read.csv("~/GitHub/Rceattle_MSE/Data/goa_temp_610_to_630_winter_300M.csv") %>%
  filter(depthclass == "Surface", hind == "yes")  %>%
  mutate(varname = "winter sst") %>%
  rename(value = mean_value_dc_610_to_630)

zoo_data <- read.csv("~/GitHub/Rceattle_MSE/Data/goa_large_zoo_610_to_630_fall_300M.csv") %>%
  filter(depthclass == "Surface", hind == "yes") %>%
  mutate(varname = "mzl") %>%
  rename(value = mean_value_dc_610_to_630)
...
quantities$R_hat[,1:10]
quantities$R[,1:10]
quantities$jnll_comp[15,]
obj$par[1:15]
obj$gr()[1:15]
obj$fn()
log(alpha)
