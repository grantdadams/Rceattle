...
##' 	\deqn{Fofl = Flimit }
##' 	\deqn{Fuse = Ftarget}
##' 	Stock status: \eqn{Alpha < SB / SB at Ftarget  \le1}
...
##' 	\deqn{Fuse=0}
##'
##' \code{hcr = 7} An HCR based on the The Southern and Eastern Scalefish and Shark Fishery (SESSF) spawner-per-recruit-based Tier 1 harvest control rule where F_Limit=F_(20%), B_Limit=SB_20, F_Target (AFMA 2017) calculated as follows:
##' 	Stock status: \eqn{ SB > SB0*Ptarget)}
##' 	\deqn{Fofl = Flimit}
##' 	\deqn{Fuse = Ftarget}
