...
build_params <- function(data_list, inits = NULL) {
    ...
    param_list$ln_srv_q_dev = matrix(0, nrow = nrow(data_list$srv_control), ncol = nyrs)   # Survey catchability deviations; n = [sum(n_srv)]
    ...
    param_list$ln_sigma_srv_index = log(data_list$srv_control$Sigma_index_prior)      # Log standard deviation of survey index time-series; n = [1, n_srv]
    param_list$ln_sigma_fsh_catch = log(data_list$fsh_control$Sigma_catch_prior)       # Log standard deviation of fishery catch time-series; n = [1, n_fsh]
