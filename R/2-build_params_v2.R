...
build_params <- function(data_list, inits = NULL) {
    ...
    # -- 3.4. Selectivity parameters FIXME - change nspp to n_srv FIXME - change order of selectivity paramters for logistic
    param_list$srv_sel_coff = suppressWarnings(matrix(0, nrow = nrow(data_list$srv_control), ncol = max(1, as.numeric(data_list$srv_control$Nselages), 
        na.rm = T)))  # Survey selectivity parameters; n = [nspp, nselages]
    ...
    param_list$fsh_sel_coff = suppressWarnings(matrix(0, nrow = nrow(data_list$fsh_control), ncol = max(1, as.numeric(data_list$fsh_control$Nselages), 
        na.rm = T)))  # Fishery age selectivity coef; n = [nspp, nselages]
    ...
    if (!is.null(inits)) {
            ...
            for (i in 1:length(param_list)) {
                ...
                if (param_names[i] %in% unique(std_dat$name)) {
                  ...
                  if (length(which(param_dim[i, ] > 1)) <= 1 | length(which(param_dim[i, ] == 1)) == ncol(param_dim)) {
                    param_list[[param_names[i]]] <- replace(param_list[[param_names[i]]], values = std_dat$value[which(std_dat$name == 
                      param_names[i])])
                  }
                  if (length(which(param_dim[i, ] > 1)) == 2) {
                    ...
                    if (param_names[i] == "init_dev") {
                      ...
                      for (j in 1:nrow(param_list[[param_names[i]]])) {
                        ...
                        param_list[[param_names[i]]][j, 1:(data_list$nages[j] - 1)] <- replace(param_list[[param_names[i]]][j, 
                          1:(data_list$nages[j] - 1)], values = init_dev[which(init_dev_lines == j)])
                      }
                    } else {
                      param_list[[param_names[i]]] <- matrix(std_dat$value[which(std_dat$name == param_names[i])], byrow = T, 
                        ncol = ncol(param_list[[param_names[i]]]), nrow = nrow(param_list[[param_names[i]]]))
                    }
                  }
                }
            }
        ...
        if (grepl(".par", inits)) {
            # std fild
            std_dat <- scan(inits, what = "", flush = T, blank.lines.skip = F, quiet = T)
            ...
            for (i in 1:length(par_file_lines)) {
                par_file_names[i] <- paste(scan(inits, skip = par_file_lines[i] - 1, flush = F, sep = "\t", nlines = 1, quiet = TRUE, 
                  what = "character", blank.lines.skip = TRUE), sep = "", collapse = " ")
            }
            ...
            par_list <- list()
            for (i in 1:length(par_file_lines)) {
                par_list[[i]] <- scan(inits, what = "numeric", flush = F, blank.lines.skip = F, skip = par_file_lines[i], nlines = ifelse(i < 
                  length(par_file_lines), par_file_lines[i + 1] - par_file_lines[i] - 1, 1), quiet = T, sep = "")
            ...
            for (i in 1:length(param_list)) {
                if (param_names[i] %in% unique(par_file_names_filtered)) {
                  ...
                  # PARAMETER_VECTOR and PARAMETER
                  if (length(which(param_dim[i, ] > 1)) <= 1 | length(which(param_dim[i, ] == 1)) == ncol(param_dim)) {
                    param_list[[param_names[i]]] <- replace(param_list[[param_names[i]]], values = unlist(par_list[grep(param_names[i], 
                      par_file_names)]))  #FIXME: this will break if the order of the saved parameters are off
                  }
                  ...
                  if (length(which(param_dim[i, ] > 1)) == 2) {
                    ...
                    if (param_names[i] == "init_dev") {
                      ...
                      param_list[[param_names[i]]] <- matrix(unlist(par_list[grep(param_names[i], par_file_names)]), byrow = T, 
                        ncol = ncol(param_list[[param_names[i]]]), nrow = nrow(param_list[[param_names[i]]]))
                    }
                  }
                }
