...
build_params <- function(data_list, inits = NULL) {
  ...
  param_list$ln_Ftarget = matrix(ifelse(is.null(data_list$Ftarget), 0, log(data_list$Ftarget)), nrow = data_list$nspp, ncol = nyrs_proj) # Future fishing mortality for projections for each species
