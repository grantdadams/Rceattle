...
compare_output <- function(rep = rep, tmp = tmp, data_list = data_list, rel_error = 0.01) {
    ...
    param_check <- list()
    tmb_names <- c("fsh_age_obs", "fsh_age_hat", "F", "pmature", "tc_biom_hat", "biomass", "srv_bio_hat", "NByage", "R", "S", 
        "srv_sel", "biomassSSB", "srv_age_obs", "srv_age_hat", "biomassByage", "fsh_sel", "M1", "tc_hat", "biomassSSB", "ration2Age", 
        "AvgN", "M2", "avail_food", "suit_other", "of_stomKir", "stomKir", "stom_div_bio2")
    admb_names <- c("fsh_age_obs", "fsh_age_hat", "F", "pmature", "obs_catch_hat", "Biomass", "srv_bio_hat", "NByage", "R", 
        "S", "srv_sel", "BiomassSSB", "srv_age_obs", "srv_age_hat", "biomassByage", "fsh_sel", "M1", "tc_hat", "BiomassSSB", 
        "ration2", "AvgN", "M2", "avail_food", "suit_other", "of_stomKir", "stomKir", "stom_div_bio2")
    
    # FIXME UPDATE indexing
        ...
        if (class(tmb_params) == "array") {
            ...
            if (length(dim(tmb_params)) == 3) {
                ...
                for (i in 1:dim(tmb_params)[3]) {
                  ...
                  diff <- abs((tmb_params[1:min(nrow(tmb_params[, , i]), nrow(admb_params)), 1:min(ncol(tmb_params[, , i]), 
                    ncol(admb_params)), i] - admb_params[1:min(nrow(tmb_params[, , i]), nrow(admb_params)), 1:min(ncol(tmb_params[, 
                    , i]), ncol(admb_params))])/admb_params[1:min(nrow(tmb_params[, , i]), nrow(admb_params)), 1:min(ncol(tmb_params[, 
                    , i]), ncol(admb_params))]) < rel_error
                  
                  param_check[[param]][1:min(nrow(tmb_params[, , i]), nrow(admb_params)), 1:min(ncol(tmb_params[, , i]), ncol(admb_params)), 
                    i] <- replace(param_check[[param]][1:min(nrow(tmb_params[, , i]), nrow(admb_params)), 1:min(ncol(tmb_params[, 
                    , i]), ncol(admb_params)), i], values = diff)
                }
            }
            # # 5D Array if(length(dim(tmb_params)) == 5){ param_check[[param]] <- replace(param_check[[param]], values = rep(NA,
            # length(param_check[[param]]))) for(sp in 1:dim(tmb_params)[1]){ for(sp_age in 1:data_list$nages[sp]){ for(yr in
            # 1:data_list$nyrs){ admb_params <- tmp[[paste(admb_param, yr, sp, sp_age, sep = '_')]] tmb_tmp <- tmb_params[sp, , sp_age,
            # ,yr] diff <- abs(tmb_tmp - admb_params) / (admb_params + 1e-16) < rel_error param_check[[param]][sp, , sp_age, ,yr] <-
            # replace( param_check[[param]][sp, , sp_age, ,yr], values = diff) } } } }
        }
    }
    ...
    like_names <- c("srv_bio_like", "srv_age_like", "eit_srv_like", "eit_age_like", "fsh_cat_like", "fsh_age_like", "fsh_sel_like", 
        NA, "srv_sel_like", NA, NA, NA, NA)
