...
mse_summary <- function(mse){
  ...
  nflts <- nrow(mse$Sim_1$OM$data_list$fleet_control)
  ...
  catch_summary_stats$Fleet_name <- c(mse$Sim_1$OM$data_list$fleet_control$Fleet_name, mse$Sim_1$OM$data_list$spnames, "All")
  catch_summary_stats$Fleet_code <- c(mse$Sim_1$OM$data_list$fleet_control$Fleet_code, mse$Sim_1$OM$data_list$spnames, "All")
  catch_summary_stats$Species <- c(mse$Sim_1$OM$data_list$fleet_control$Species, mse$Sim_1$OM$data_list$spnames, "All")
  ...
  for(sp in 1:nspp){
    ...
    catch_summary_stats$`% Years closed`[sp+nflts] <- mean(sapply(catch_list_tmp, function(x) 
      length(x$Catch < 1) # Using less than 1 here just in case super small catches and fishery is effectively close
      /length(projyrs) * 100))
  }
  ...
  catch_summary_stats$`Average Catch`[nspp + nflts + 1] <- mean(sapply(mse, function(x) 
    x[[1]]$data_list$fsh_biom$Catch[which(x[[1]]$data_list$fsh_biom$Year %in% projyrs)]), na.rm = TRUE)
  ...
  catch_summary_stats$`% Years closed`[nspp + nflts + 1] <- mean(sapply(catch_list_tmp, function(x) 
    length(x$Catch < 1) # Using less than 1 here just in case super small catches and fishery is effectively close
    /length(projyrs) * 100))
