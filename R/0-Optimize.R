...
Optimize = function(obj, fn = obj$fn, gr = obj$gr, startpar = obj$par, lower = rep(-Inf, length(startpar)), upper = rep(Inf,
    ...
    parameter_estimates[["AIC"]] = Rceattle::TMBAIC(opt = parameter_estimates)
    if (n != Inf) {
        parameter_estimates[["AICc"]] = Rceattle::TMBAIC(opt = parameter_estimates, n = n)
        parameter_estimates[["BIC"]] = Rceattle::TMBAIC(opt = parameter_estimates, p = log(n))
    }
    parameter_estimates[["diagnostics"]] = data.frame(Param = names(startpar), starting_value = startpar, Lower = lower, MLE = parameter_estimates$par,
