...
b1 <- microbenchmark({
  obj <- RTMB::MakeADFun(function(p)do.call(func,p), parameters, random=c("U"), DLL="sam_rtmb", silent = TRUE)
})
...
o1 <- microbenchmark({opt <- nlminb(obj$par, obj$fn, obj$gr, lower=lower, upper=upper)})
r1 <- microbenchmark({rep <- sdreport(obj)})


f1 <- microbenchmark({obj$fn()})
g1 <- microbenchmark({obj$gr()})
