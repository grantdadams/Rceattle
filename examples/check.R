...
sample_yrs <- sapply(sampling_period, function(x) seq(from = em$data_list$endyr + x, to = em$data_list$projyr,  by = x))
fleet_id <- sample_yrs
...
for(sim in 1:nsim){
  ...
  for(k in 1:(length(assess_yrs))){
    ...
    new_comp_data <- sim_dat$comp_data[which(abs(sim_dat$comp_data$Year) %in% years_include & sim_dat$comp_data$Fleet_code %in% years_include$Fleet_code),]
  ...
  if(!is.null(filename)){
    saveRDS(list(OM = om_use, EM = Rceattle_EM_list[[sim]]), file = paste0(filename, "EMs_from_OM_Sim_",sim, ".rds"))
