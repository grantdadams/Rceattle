...
-   `family` measurement-error distribution for each env-data variable (can be length 1; default `"fixed"`). `recdevs<sp>` columns are always `fixed`. Requires `dsem` 3.0.0.
...
print(model1$estimated_params$dsem_beta_z)
```
...
summ$Estimate <- model1$estimated_params$dsem_beta_z
knitr::kable(summ)
```
