...
# Then the model can be fit by setting `msmMode = 0` using the `Rceattle` function:
BS2017SS$fleet_control$proj_F_prop <- c(rep(1,3), rep(0,4))
ss_run_F <- Rceattle::fit_mod(data_list = BS2017SS,
                              inits = NULL, # Initial parameters = 0
                              file = NULL, # Don't save
                              estimateMode = 0, # Estimate
...
# For the a multispecies model we from the single species parameters.
BS2017MS$fleet_control$proj_F_prop <- c(rep(1,3), rep(0,4))
ms_run_F <- Rceattle::fit_mod(data_list = BS2017MS,
                              inits = ss_run$estimated_params, # Initial parameters from single species ests
                              M1Fun = build_M1(M1_model = 1,
                                               updateM1 = TRUE,
                                               M1_use_prior = FALSE,
...
# Then the model can be fit by setting `msmMode = 0` using the `Rceattle` function:
BS2017SS$fleet_control$proj_F_prop <- c(rep(1,3), rep(0,4))
ss_run_4010_F <- Rceattle::fit_mod(data_list = BS2017SS,
                              inits = NULL, # Initial parameters = 0
                              file = NULL, # Don't save
                              estimateMode = 0, # Estimate
...
# For the a multispecies model we from the single species parameters.
BS2017MS$fleet_control$proj_F_prop <- c(rep(1,3), rep(0,4))
ms_run_4010_F <- Rceattle::fit_mod(data_list = BS2017MS,
                              inits = ss_run$estimated_params, # Initial parameters from single species ests
                              M1Fun = build_M1(M1_model = 1,
                                               updateM1 = TRUE,
                                               M1_use_prior = FALSE,
...
##############################################################################
ss_run_4010_catch <- catch_hcr(ss_run_F)
ms_run_4010_catch <- catch_hcr(ms_run_F)
...
#'
#'
catch_hcr <- function(model = ss_run, ptarget = 0.4, plimit = 0.1, assessment_period = 1, cap = NULL){
  ...
  for(k in 1:(length(assess_yrs))){
    ...
    model <- fit_mod(
      ...
      M1Fun =     build_M1(M1_model= model$data_list$M1_model,
                           ...
                           M1_prior_sd = model$data_list$M1_prior_sd), # Dont update M1 from data, fix at previous parameters
      loopnum = 3,
      phase = NULL,
      getsd = FALSE,
      verbose = 0,
